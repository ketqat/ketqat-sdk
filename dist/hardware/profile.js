...
export function couplingAdjacency(profile) {
    const inoperable = new Set(profile.qubits.filter((qubit) => qubit.operational === false).map((qubit) => qubit.index));
    ...
    for (const coupling of profile.couplings) {
        if (inoperable.has(coupling.control) || inoperable.has(coupling.target))
            continue;
...
export function linearTopology(qubitCount) {
...
export function gridTopology(rows, columns) {
    const couplings = [];
    const index = (row, column) => row * columns + column;
    for (let row = 0; row < rows; row += 1) {
        for (let column = 0; column < columns; column += 1) {
            if (column + 1 < columns)
                couplings.push({ control: index(row, column), target: index(row, column + 1) });
            if (row + 1 < rows)
                couplings.push({ control: index(row, column), target: index(row + 1, column) });
        }
    }
    return couplings;
}
