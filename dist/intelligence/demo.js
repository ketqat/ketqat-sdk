...
import { EconomicModelSchema, HardwareModelSnapshotSchema, ResourceScenarioSchema, presetScenarios, } from "./scenario.js";
import { workloadFromResourceEstimate } from "./workload.js";
/**
...
export function demoWorkload() {
    ...
    return workloadFromResourceEstimate({
        ...
        description: "A small Clifford+T circuit with Toffoli gates, used to demonstrate the KetQat Intelligence pipeline " +
...
export function demoBaseline() {
    return ClassicalBaselineSchema.parse({
        ...
        measured_on: null,
        evidence_url: null,
        evidence_note: "Illustrative figure for the demonstration, not a measurement of any classical system. Supplied so the " +
 ...
 */
export function demoEconomicModel() {
    return EconomicModelSchema.parse({
        ...
        source: "A hypothetical rate supplied for demonstration. No price for fault-tolerant quantum machine time exists to " +
...
export function demoScenarios() {
    const presets = presetScenarios({ runtimeTarget: 86400 });
    const base = presets[1];
    const aboveThreshold = ResourceScenarioSchema.parse({
        ...base,
        name: "Above threshold",
        preset: "CUSTOM",
        rationale: "A device whose physical error rate sits above the surface-code threshold. Included to demonstrate that the " +
