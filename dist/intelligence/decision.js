...
const PARAMETER_NOTES = {
    PHYSICAL_ERROR_RATE: "A device property. This uncertainty shrinks by measuring or improving the hardware.",
    LOGICAL_ERROR_PREFACTOR: "A fitted constant with no clear provenance. This uncertainty does not shrink by buying a better device.",
    LAYOUT_MODEL: "A convention about routing space. The bare-register reading is an underestimate, not an alternative.",
...
export function assessDecision(input) {
    ...
    if (workload.is_demo)
        reasons.add("DEMO_FIXTURE");
    // ------------------------------------------------- scientific feasibility
    const scientificCodes = [];
    let scientificStatus = "SATISFIED";
    let scientificExplanation = `A code distance of ${estimate.code_distance.value} meets the ${scenario.error_budget} error budget at a ` +
    ...
    if (estimate.infeasibility_code === "ABOVE_SURFACE_CODE_THRESHOLD") {
        ...
        scientificExplanation = estimate.infeasibility_reason;
    }
    ...
    if (workload.logical.unsupported_for_ft_count > 0) {
        ...
        if (workload.logical.t_count === 0 && workload.logical.toffoli_count === 0) {
            ...
            missingEvidence.push("A Clifford+T synthesis of the non-Clifford, non-T gates in this circuit. Without it the T count, and " +
    ...
    let engineeringStatus = "SATISFIED";
    let engineeringExplanation;
    if (!estimate.feasible) {
        ...
        missingEvidence.push("A distillation configuration that reaches the target magic-state error. Until one exists the factory " +
            "footprint, and therefore the machine size, is not computed.");
        nextMeasurement.push("Lower the raw magic-state error, loosen the target state error, or select a distillation protocol that " +
            "reaches the target.");
    }
    else {
    ...
    if (capacity === null) {
        ...
        missingEvidence.push("The physical qubit capacity of the device under consideration. Without it, 'does it fit' has no answer.");
        nextMeasurement.push("State the physical qubit capacity of the target device, with a source and a date.");
    }
    else if (!isKnown(estimate.total_physical_qubits)) {
        hardwareStatus = "INSUFFICIENT_EVIDENCE";
        hardwareExplanation =
    ...
    const basisCode = scenario.hardware.basis === "OBSERVATION"
        ? "HARDWARE_BASIS_OBSERVATION"
        : scenario.hardware.basis === "ROADMAP"
            ? "HARDWARE_BASIS_ROADMAP"
            : "HARDWARE_BASIS_USER_ASSUMPTION";
    hardwareCodes.push(basisCode);
    if (scenario.hardware.confidence === "LOW")
        hardwareCodes.push("HARDWARE_CONFIDENCE_LOW");
    if (scenario.hardware.basis !== "OBSERVATION") {
        ...
        missingEvidence.push("A dated observation of a real device meeting these physical error rate and cycle time parameters.");
    }
    hardwareCodes.forEach((code) => reasons.add(code));
    // ---------------------------------------------------- economic readiness
    const economicCodes = [];
    let economicStatus = "INSUFFICIENT_EVIDENCE";
    ...
    if (!hasBaseline) {
        ...
        economicExplanation =
            `${INSUFFICIENT_ECONOMIC_EVIDENCE}: no classical baseline was supplied, so there is nothing to be faster or ` +
                "cheaper than.";
        missingEvidence.push("A classical baseline: runtime, cost, hardware, problem size, solution quality, and the date it was measured.");
        nextMeasurement.push("Measure the current classical solution on the real problem size and record the hardware and date.");
    }
    else {
        ...
        if (!hasEconomics) {
            ...
            missingEvidence.push("A quantum cost model: what a machine-second, or a physical-qubit-second, is assumed to cost, and on whose authority.");
            nextMeasurement.push("State a quantum machine-cost assumption, even a hypothetical one, so the economic thresholds become computable.");
        }
    ...
    const speedup = isKnown(threshold.runtime_speedup_over_classical)
        ? threshold.runtime_speedup_over_classical.value
        : null;
    const costRatio = isKnown(threshold.cost_ratio_to_classical)
    ...
    if (costRatio !== null) {
        economicCodes.push(costRatio < 1 ? "PROJECTED_COST_BELOW_CLASSICAL" : "PROJECTED_COST_ABOVE_CLASSICAL");
    }
    if (hasBaseline && hasEconomics && (speedup !== null || costRatio !== null)) {
        const parts = [];
        if (speedup !== null) {
            parts.push(`a projected ${speedup.toPrecision(3)}x runtime ratio against the ` +
    ...
    // --------------------------------------------------- evidence confidence
    const evidenceCodes = [];
    evidenceCodes.push(workload.logical_counts_evidence === "DERIVED"
        ? "WORKLOAD_COUNTS_DERIVED_FROM_CIRCUIT"
        : "WORKLOAD_COUNTS_USER_PROVIDED");
    let evidenceScore = 0;
    if (workload.logical_counts_evidence === "DERIVED")
        evidenceScore += 1;
    if (scenario.hardware.basis === "OBSERVATION")
        evidenceScore += 1;
    if (scenario.hardware.confidence === "HIGH")
        evidenceScore += 1;
    if (hasBaseline && baseline.evidence === "MEASURED")
        evidenceScore += 1;
    if (hasEconomics && scenario.economics?.basis === "PUBLISHED_QUOTE")
        evidenceScore += 1;
    const evidenceGrade = evidenceScore >= 4 ? "HIGH" : evidenceScore >= 2 ? "MEDIUM" : "LOW";
    const evidenceStatus = evidenceGrade === "LOW" ? "NOT_SATISFIED" : "SATISFIED";
    const evidenceExplanation = `Logical counts are ${workload.logical_counts_evidence}; hardware parameters are ${scenario.hardware.basis} ` +
    ...
    const sensitivityStatus = sensitivityGrade === "UNCHARACTERIZED"
        ? "INSUFFICIENT_EVIDENCE"
        : sensitivityGrade === "HIGH"
            ? "NOT_SATISFIED"
            : "SATISFIED";
    ...
    if (errorRateSpread !== null && errorRateSpread >= 10)
        sensitivityCodes.push("HIGH_SENSITIVITY_TO_ERROR_RATE");
    if (modelSpread !== null && modelSpread >= 2)
        sensitivityCodes.push("HIGH_MODEL_SPREAD");
    if (layoutSpread !== null && layoutSpread >= 2)
        sensitivityCodes.push("HIGH_LAYOUT_SPREAD");
    if (worstSpread !== null && worstSpread < 10)
        sensitivityCodes.push("SENSITIVITY_WITHIN_ONE_ORDER");
    const sensitivityExplanation = worstSpread === null
        ...
        : `The widest spread comes from the ${spreads[0].parameter}: a ${worstSpread.toPrecision(3)}x range in total ` +
            ...
            (modelSpread !== null && modelSpread >= 2
                ...
                : "");
    if (modelSpread !== null && modelSpread >= 2) {
        uncertaintyWarnings.push(`Choosing the other published logical-error prefactor changes the total physical qubit count by ` +
            `${modelSpread.toPrecision(3)}x. The prefactor is fitted and its provenance is weak; no measurement narrows it.`);
    }
    if (layoutSpread !== null && layoutSpread >= 2) {
        uncertaintyWarnings.push(`The two layout conventions differ by ${layoutSpread.toPrecision(3)}x in total physical qubits. These are not ` +
            "equally defensible: routing space is real hardware, so the bare-register figure is an underestimate.");
    }
    if (errorRateSpread !== null && errorRateSpread >= 10) {
        uncertaintyWarnings.push(`A factor-of-four change in the physical error rate changes the machine size by ${errorRateSpread.toPrecision(3)}x. ` +
    ...
    if (economicComparisonAvailable &&
    ...
    if (status === "INFEASIBLE_UNDER_ASSUMPTIONS") {
        sentences.push(estimate.infeasibility_reason);
    }
    else if (status === "INSUFFICIENT_EVIDENCE") {
        sentences.push("The computation cannot be assessed under this model because a required component could not be computed.");
        ...
        sentences.push(`Under this model the computation needs ${machine} and ${time} at code distance ` +
            `${estimate.code_distance.value}.`);
        if (!hasBaseline || !hasEconomics) {
            sentences.push(`The computation is technically feasible under this model, but no economic conclusion can be made because ` +
                (!hasBaseline
    ...
    if (isKnown(threshold.max_cycle_time_to_beat_classical_runtime)) {
        sentences.push(`A surface-code cycle below ${threshold.max_cycle_time_to_beat_classical_runtime.value.toPrecision(3)} ns ` +
            "would be required to beat the supplied classical runtime.");
    }
    else if (isKnown(threshold.max_cycle_time_for_runtime_target)) {
        sentences.push(`A surface-code cycle below ${threshold.max_cycle_time_for_runtime_target.value.toPrecision(3)} ns would be ` +
            `required to finish within the ${scenario.runtime_target} s target.`);
    }
    if (workload.is_demo) {
        sentences.push("This assessment is built from a demonstration fixture and is not evidence about any real workload or device.");
    }
    if (binding === "MAGIC_STATE_THROUGHPUT") {
        nextMeasurement.push("Characterize magic-state factory throughput on the target architecture: it, not logical-qubit count, sets " +
            "the runtime here.");
    }
    if (binding === "PHYSICAL_ERROR_RATE" || (errorRateSpread !== null && errorRateSpread >= 10)) {
        nextMeasurement.push("Measure the two-qubit physical error rate on the target device. It is the parameter this result depends on " +
            "most, and it is measurable.");
    }
    if (nextMeasurement.length === 0) {
        nextMeasurement.push("Replace the assumed device parameters with a dated measurement of a real device, so the result rests on an " +
