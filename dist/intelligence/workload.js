...
export const WorkloadSourceSchema = z
    ...
    if (source.kind === "OPENQASM3" && !source.openqasm3) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: "An OPENQASM3 workload must carry its source, so the counts can be recomputed rather than believed.",
            path: ["openqasm3"],
        });
    }
});
...
export const QuantumWorkloadSchema = z
     ...
     */
    logical_counts_evidence: EvidenceClassSchema.refine((value) => value === "DERIVED" || value === "USER_PROVIDED" || value === "MODELLED", {
        message: "Logical gate counts are DERIVED (parsed), USER_PROVIDED (supplied), or MODELLED (from an analytic formula). " +
