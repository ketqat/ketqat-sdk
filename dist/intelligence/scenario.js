...
export const FactoryAssumptionsSchema = z
    ...
    if (factory.factory_distance !== null &&
        (factory.factory_distance < 3 || factory.factory_distance % 2 === 0)) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: `A surface-code distance must be an odd integer of at least 3, got ${factory.factory_distance}. ` +
    ...
    if (factory.protocol === "FIFTEEN_TO_ONE" && factory.target_state_error >= factory.raw_state_error) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: "The target state error is not below the raw state error, so distillation has nothing to do. " +
...
export const EconomicModelSchema = z
    ...
    if (model.machine_cost_per_second === null && model.physical_qubit_cost_per_second === null) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: "An economic model with no cost at all is not a model. Supply one of the two rates, or omit the model " +
 ...
 */
export const CONVENTIONAL_QEC_MODEL = QecModelSnapshotSchema.parse({
    ...
    source: "Rotated surface code under circuit-level depolarizing noise; threshold ~1% and p_L = 0.03 (p/p_th)^((d+1)/2) " +
...
function factoryFor(rawStateError) {
...
const PRESET_SHAPES = [
    {
        name: "Conservative",
        preset: "CONSERVATIVE",
        rationale: "Device parameters at the pessimistic end of what good superconducting hardware has demonstrated. " +
        ...
        hardware: referenceHardware({
            ...
            cycleTimeNs: 1000,
            capacity: null,
            source: "A conventional pessimistic reading of demonstrated two-qubit error rates on superconducting devices, " +
    ...
    },
    {
        name: "Base",
        preset: "BASE",
        rationale: "The parameters most fault-tolerance resource analyses use: a 1e-3 physical error rate and a 1 microsecond " +
        ...
        hardware: referenceHardware({
            ...
            cycleTimeNs: 1000,
            capacity: null,
            source: "The physical error rate and cycle time used as the standard case in surface-code resource analyses, " +
    ...
    },
    {
        name: "Optimistic",
        preset: "OPTIMISTIC",
        rationale: "Device parameters an order of magnitude better than the standard case, and a five-times faster cycle. " +
        ...
        hardware: referenceHardware({
            ...
            cycleTimeNs: 200,
            capacity: null,
            source: "An improvement target stated for comparison, not a measurement or a vendor roadmap. No device is claimed " +
 ...
 */
export function presetScenarios(options = {}) {
    ...
    return PRESET_SHAPES.map((shape) => ResourceScenarioSchema.parse({
        schema_version: INTELLIGENCE_SCHEMA_VERSION,
        name: shape.name,
        preset: shape.preset,
        revision: 1,
 ...
 */
export function scenariosComparable(left, right) {
    const reasons = [];
    if (left.estimator.name !== right.estimator.name) {
        reasons.push(`Different estimators ('${left.estimator.name}' and '${right.estimator.name}') define these quantities differently.`);
    }
    else if (left.estimator.version !== right.estimator.version) {
        reasons.push(`Same estimator at different versions ('${left.estimator.version}' and '${right.estimator.version}').`);
    }
    ...
    if (left.qec.threshold !== right.qec.threshold || left.qec.prefactor !== right.qec.prefactor) {
        reasons.push(`Different logical-error models ('${left.qec.prefactor_model}' and '${right.qec.prefactor_model}'). ` +
    ...
    if (left.error_budget !== right.error_budget) {
        reasons.push(`Different error budgets (${left.error_budget} and ${right.error_budget}). A cheaper result under a looser ` +
            "budget is not a better result.");
    }
    if (left.decomposition.toffoli_t_cost !== right.decomposition.toffoli_t_cost) {
        reasons.push(`Different Toffoli decompositions (${left.decomposition.toffoli_t_cost} T and ${right.decomposition.toffoli_t_cost} T).`);
    }
    return { comparable: reasons.length === 0, reasons };
}
/** Produce the next revision of a scenario, superseding the current one. */
export function reviseScenario(current, changes, currentHash) {
