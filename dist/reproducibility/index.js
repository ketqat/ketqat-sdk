...
const excludedByVersion = {
    1: new Set([...identityKeys, HASH_VERSION_KEY]),
    2: new Set([...identityKeys, HASH_VERSION_KEY, ...timingKeys]),
};
function excludedFor(version) {
    const excluded = excludedByVersion[version];
    if (!excluded) {
        throw new Error(`Unknown reproducibility hash version ${version}. Known versions: ${Object.keys(excludedByVersion).join(", ")}.`);
    }
    return excluded;
}
/**
 ...
 */
export function hashVersionOf(input) {
    const recorded = input[HASH_VERSION_KEY];
...
export function canonicalResearchJson(input, version = CURRENT_HASH_VERSION) {
    return JSON.stringify(canonicalize(input, excludedFor(version)));
}
export function calculateReproducibilityHash(input, version = CURRENT_HASH_VERSION) {
 ...
 */
export function canonicalJsonForExcludedKeys(input, excluded) {
    ...
    return JSON.stringify(canonicalize(input, excluded));
}
