...
import { ContentHashSchema, STUDY_SCHEMA_VERSION, StudyPositionSchema, TextFieldSchema, } from "./common.js";
import { StudyCriterionSchema, duplicateCriterionIds } from "./criteria.js";
import { studySelfHash } from "./hash.js";
import { StudyIdSchema } from "./identity.js";
import { OpenQuestionSchema, duplicateQuestionIds, isQuestionOutstanding, isSettledWithoutAnAnswer, questionResolutionRule, questionTargetsField, } from "./questions.js";
...
export const SpecificationSuccessCriterionSchema = z
    .object({
    ...
    if (stated && criterion.statement.origin === "CONFIRMED" && criterion.predicate === null) {
        context.addIssue({
            ...
            message: "A confirmed success criterion carries the predicate an orchestrator evaluates. Prose alone leaves the " +
...
const stateOf = (known, confirmed) => {
    if (known)
        return confirmed ? "ANSWERED" : "INFERRED";
    return confirmed ? "CONFIRMED_UNKNOWN" : "UNANSWERED";
};
/** Every answerable field of a specification, with the state it is in. */
export function specificationFieldStates(specification) {
    const text = (path, label, field) => ({
        path,
        label,
        state: stateOf(field.value !== null, field.origin === "CONFIRMED"),
    });
    const measured = (path, label, field) => ({
        path,
        label,
    ...
    return Object.freeze([
        ...
        ...specification.success_criteria.map((criterion, index) => text(`success_criteria[${index}]`, `success criterion ${index + 1}`, criterion.statement)),
...
export const ProblemSpecificationSchema = z
    ...
    for (const duplicate of duplicateQuestionIds(specification.open_questions)) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: `Two questions are filed under ${duplicate}. An answer recorded against that id would belong to both, ` +
    ...
    for (const status of states) {
        const targeting = specification.open_questions.filter((question) => questionTargetsField(question, status.path));
        if (status.state === "UNANSWERED" || status.state === "INFERRED") {
            if (!targeting.some(isQuestionOutstanding)) {
                context.addIssue({
                    code: z.ZodIssueCode.custom,
                    message: `The ${status.label} is ${status.state === "INFERRED" ? "the system's own proposal" : "unanswered"} ` +
        ...
        if (status.state === "CONFIRMED_UNKNOWN" && !targeting.some(isSettledWithoutAnAnswer)) {
            context.addIssue({
                code: z.ZodIssueCode.custom,
                message: `The ${status.label} carries no value and is marked confirmed, and no question targeting ` +
    ...
    for (const question of specification.open_questions) {
        ...
        if (rule.answered && state !== "ANSWERED") {
            context.addIssue({
                code: z.ZodIssueCode.custom,
                message: `Question ${question.question_id} is recorded as answered and ${question.targets} is ${state}. ` +
        ...
        if (!rule.answered && !rule.outstanding && state !== "CONFIRMED_UNKNOWN") {
            context.addIssue({
                code: z.ZodIssueCode.custom,
                message: `Question ${question.question_id} is settled without an answer and ${question.targets} is ${state}. ` +
 ...
 */
export function reviseSpecification(current, changes, currentHash, latestRevision) {
    const refusal = studyRevisionRefusal("problem_specification", `problem specification revision ${current.revision}`, current, currentHash, latestRevision);
    if (refusal !== null)
