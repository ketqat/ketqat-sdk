import { z } from "zod";
import { FieldOriginSchema, StudyQuantitySchema } from "./common.js";
/**
...
const family = (dimension, units, means, foreign = []) => Object.freeze({
    dimension,
 ...
 * is a member.
 */
export const STUDY_UNIT_FAMILIES = Object.freeze([
    family("TIME", ["nanoseconds", "microseconds", "milliseconds", "seconds", "minutes", "hours", "days"], "how long something takes, or how long it may take"),
    ...
    family("MEMORY", ["bytes", "kibibytes", "mebibytes", "gibibytes", "tebibytes"], "storage or working memory, in binary-prefixed units"),
    ...
    family("ACCURACY", ["relative error", "percent", "probability", "ratio"], "how close an answer has to be, said without saying close in what"),
    family("PROBLEM_SIZE", [], "the size of the instance an answer has to hold for, counted in whatever the instance is made of", ["TIME", "MONEY", "CREDITS", "MEMORY", "ACCURACY"]),
]);
/** The working lookup, module-private and built from the frozen tuple. */
const familiesByDimension = new Map(STUDY_UNIT_FAMILIES.map((entry) => [entry.dimension, entry]));
...
export function studyUnitFamily(dimension) {
...
export function studyUnitSchema(dimension) {
    const entry = studyUnitFamily(dimension);
    if (!entry.open) {
        return z.enum(entry.units, {
            errorMap: () => ({
                message: `A ${entry.dimension} quantity is ${entry.means}. Its units are: ${entry.units.join(", ")}. ` +
    ...
    return z.string().min(1).regex(openUnitPattern(entry), {
        message: `A ${entry.dimension} quantity is ${entry.means}, so its unit may be anything except a unit belonging to ` +
            `${entry.foreign.join(", ")}.`,
    });
}
/**
 ...
 */
const studyQuantityObject = StudyQuantitySchema.innerType();
const quantityByDimension = new Map();
export function dimensionedQuantitySchema(dimension) {
...
export const STUDY_FIELD_DIMENSIONS = Object.freeze([
    Object.freeze({ record_kind: "problem_specification", field: "accuracy_requirement", dimension: "ACCURACY" }),
    Object.freeze({ record_kind: "problem_specification", field: "runtime_constraint", dimension: "TIME" }),
    Object.freeze({ record_kind: "problem_specification", field: "budget_constraint", dimension: "MONEY" }),
    Object.freeze({ record_kind: "problem_specification", field: "problem_size", dimension: "PROBLEM_SIZE" }),
    Object.freeze({ record_kind: "study_plan", field: "expected_runtime", dimension: "TIME" }),
    Object.freeze({ record_kind: "study_plan", field: "expected_credits", dimension: "CREDITS" }),
]);
