...
export function containsUngroundedNumber(text) {
 ...
 */
export const StudyReportSegmentSchema = z
    .object({
    ...
    for (const key of payloadKeys) {
        ...
        if (needed.has(key) && !present) {
            context.addIssue({
                ...
                message: `A ${segment.kind} segment must carry ${key}. A segment that names nothing renders as nothing, and a ` +
        ...
        if (!needed.has(key) && present) {
            context.addIssue({
                code: z.ZodIssueCode.custom,
                message: `A ${segment.kind} segment must leave ${key} null. A segment carrying two payloads is rendered one way ` +
...
export const StudyReportSectionSchema = z
    .object({
 ...
 */
export const StudyCommentaryBlockSchema = z
    .object({
    ...
    .strict();
export const StudyReportDocumentSchema = z
    .object({
...
function marker(index, key, prefix) {
 ...
 * reader quotes.
 */
function renderSection(section, context, markers, cells) {
    ...
    for (const segment of section.segments) {
        ...
        switch (segment.kind) {
            ...
            case "CLAIM_REF": {
                ...
                paragraph.push(`**${node?.label ?? "(claim not carried)"}**` +
                    (segment.node_hash === null
                        ? ""
                        : marker(markers.nodes, segment.node_hash, "n")));
                break;
            }
            case "QUANTITY_REF": {
                const node = segment.node_hash === null ? undefined : context.nodes.get(segment.node_hash);
                paragraph.push(`${renderQuantity(node?.quantity ?? null)}` +
                    (segment.node_hash === null
                        ? ""
                        : marker(markers.nodes, segment.node_hash, "n")));
                break;
            ...
            case "CITATION_REF": {
                ...
                paragraph.push(marker(markers.citations, index, "c"));
                break;
            }
            case "LIMITATION_REF": {
                const index = segment.limitation_index ?? 0;
                paragraph.push(marker(markers.limitations, index, "l"));
                break;
            }
            case "TABLE_REF": {
            ...
            case "FIGURE_REF": {
                const figure = context.figures.find((candidate) => candidate.figure_id === segment.figure_id);
                if (figure === undefined) {
 ...
 */
export function renderReportMarkdown(document, context) {
    const markers = {
        nodes: new Map(),
        citations: new Map(),
        limitations: new Map(),
    ...
    const blocks = document.sections.map((section) => renderSection(section, context, markers, cells));
 ...
 */
export function groundedProseFindings(document, section = "report") {
    const findings = [];
    document.sections.forEach((current, sectionIndex) => {
        if (containsUngroundedNumber(current.title)) {
            findings.push(finding("VERIFIED_PROSE_NOT_GROUNDED", studyPath(section, "sections", sectionIndex, "title"), "This section title carries a number standing on its own, which is a figure in the report reached " +
        ...
        current.segments.forEach((segment, segmentIndex) => {
            if (segment.kind !== "PROSE" && segment.kind !== "HEADING")
                return;
            if (segment.text === null || !containsUngroundedNumber(segment.text))
                return;
            findings.push(finding("VERIFIED_PROSE_NOT_GROUNDED", studyPath(section, "sections", sectionIndex, "segments", segmentIndex, "text"), "This text carries a number standing on its own. A figure typed into a verified section is " +
 ...
 * not.
 */
export function reportFindings(document, context, section = "report") {
    const findings = groundedProseFindings(document, section);
    ...
    document.sections.forEach((current, sectionIndex) => {
        if (sectionIds.has(current.section_id)) {
            findings.push(finding("REPORT_DUPLICATE_ID", studyPath(section, "sections", sectionIndex, "section_id"), `Two sections share the id ${JSON.stringify(current.section_id)}, so a reader following a link to it ` +
                "arrives at whichever the renderer reached first."));
        }
        sectionIds.add(current.section_id);
        current.segments.forEach((segment, segmentIndex) => {
            const at = (...parts) => studyPath(section, "sections", sectionIndex, "segments", segmentIndex, ...parts);
            if (segment.kind === "CLAIM_REF" || segment.kind === "QUANTITY_REF") {
                ...
                if (node === undefined) {
                    findings.push(finding("REPORT_REFERENCE_UNRESOLVED", at("node_hash"), `This segment reads node ${hash ?? "(none)"}, and the package does not carry it. The section renders ` +
                ...
                if (segment.kind === "QUANTITY_REF" && node.quantity === null) {
                    findings.push(finding("REPORT_REFERENCE_KIND_MISMATCH", at("node_hash"), `This segment renders a number and names the ${node.kind} node ${JSON.stringify(node.label)}, which ` +
                ...
                if (segment.kind === "CLAIM_REF" && node.kind !== "claim") {
                    findings.push(finding("REPORT_REFERENCE_KIND_MISMATCH", at("node_hash"), `This segment renders an assertion and names a ${node.kind} node. A measurement rendered as a claim ` +
            ...
            if (segment.kind === "CITATION_REF") {
                ...
                if (index >= 0 && index < context.citations.length)
                    return;
                findings.push(finding("REPORT_REFERENCE_UNRESOLVED", at("citation_index"), `This segment cites reference ${index}, and the package carries ${context.citations.length}. A ` +
            ...
            if (segment.kind === "LIMITATION_REF") {
                ...
                if (index >= 0 && index < context.limitations.length)
                    return;
                findings.push(finding("REPORT_REFERENCE_UNRESOLVED", at("limitation_index"), `This segment cites limitation ${index}, and the package carries ${context.limitations.length}. A ` +
            ...
            if (segment.kind === "TABLE_REF" && !tableIds.has(segment.table_id ?? "")) {
                findings.push(finding("REPORT_REFERENCE_UNRESOLVED", at("table_id"), `This segment renders table ${JSON.stringify(segment.table_id ?? "")}, and the package carries no ` +
                    "table with that id."));
                return;
            }
            if (segment.kind === "FIGURE_REF" && !figureIds.has(segment.figure_id ?? "")) {
                findings.push(finding("REPORT_REFERENCE_UNRESOLVED", at("figure_id"), `This segment renders figure ${JSON.stringify(segment.figure_id ?? "")}, and the package carries no ` +
                    "figure with that id."));
            }
        });
    });
    document.commentary.forEach((block, index) => {
        if (commentaryIds.has(block.commentary_id)) {
            findings.push(finding("REPORT_DUPLICATE_ID", studyPath(section, "commentary", index, "commentary_id"), `Two commentary blocks share the id ${JSON.stringify(block.commentary_id)}.`));
        }
        commentaryIds.add(block.commentary_id);
    });
    const reportBytes = document.sections.reduce((total, current) => total +
        utf8ByteLength(current.title) +
        current.segments.reduce((sum, segment) => sum + utf8ByteLength(segment.text ?? ""), 0), 0);
    const overReport = limitFinding(studyPath(section, "sections"), "bytes of report text", reportBytes, STUDY_PACKAGE_LIMITS.max_report_bytes);
    if (overReport !== null)
        findings.push(overReport);
    const commentaryBytes = document.commentary.reduce((total, block) => total + utf8ByteLength(block.title) + utf8ByteLength(block.text), 0);
    const overCommentary = limitFinding(studyPath(section, "commentary"), "bytes of commentary", commentaryBytes, STUDY_PACKAGE_LIMITS.max_commentary_bytes);
    if (overCommentary !== null)
        findings.push(overCommentary);
    return findings;
}
