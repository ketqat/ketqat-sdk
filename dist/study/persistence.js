 ...
 */
export const STUDY_PERSISTENCE_INVARIANTS = Object.freeze([
    Object.freeze({
        name: "study_id_unique",
        record_kinds: Object.freeze(["study"]),
        key: Object.freeze(["study_id"]),
        kind: "unique",
        sdk_checks: "that a study id is a lowercase version 4 UUID and that a record carrying one is shaped like a study. It " +
            "cannot know whether some other row already has that id.",
        persistence_must: "keep study_id unique across every study, and generate it once with newStudyId rather than deriving it " +
            "from anything about the study.",
        if_violated: "two studies share an id, so every study_ref in the graph resolves to both of them and neither can be " +
    ...
    Object.freeze({
        ...
        kind: "unique",
        sdk_checks: "that the trail it was handed numbers itself 1..n in order, that each event names the hash of the one " +
            "before it, and that every event in it belongs to the same study.",
        persistence_must: "hold a unique index on (study_ref, sequence). Two events written at one sequence are two trails, and each " +
            "one verifies on its own.",
        if_violated: "two internally consistent histories exist for one study, and a reader handed either is told it is intact. " +
    ...
    Object.freeze({
        ...
        sdk_checks: "that the trail ends at the head the caller passed to appendStudyEvent, which is required rather than " +
            "optional precisely so a stale read is refused before an event is built on it.",
        persistence_must: "make the insert conditional on the stored head still being previous_event_hash, and fail the write rather " +
        ...
        if_violated: "two events claim the same predecessor. The chain each one forms is valid, so nothing downstream can tell " +
    ...
    Object.freeze({
        ...
        kind: "unique",
        sdk_checks: "that revision n+1 names revision n in supersedes, that the record it was built from hashes to the hash " +
            "written on it, and that the caller's asserted hash is that same hash.",
        persistence_must: "hold a unique index on (study_ref, revision) per record kind. Two revision 3s of one plan are two " +
            "branches, and a confirmation of either verifies.",
        if_violated: "a confirmation names a revision 3 that exists twice, so what was approved and what runs can differ while " +
    ...
    Object.freeze({
        ...
        kind: "compare_and_set",
        sdk_checks: "that the latest revision the caller supplied is the record being revised, when the caller supplies one. " +
            "Without it the SDK can only see that the record in hand is intact, not that it is still the newest.",
        persistence_must: "make the insert conditional on the stored latest revision still being the one named in supersedes. This " +
            "is the branching case: two callers reading revision 2 at the same moment both produce a well-formed " +
            "revision 3.",
        if_violated: "the plan a user confirmed and the plan a task was authorised against are two different revision 3s, and " +
    ...
    Object.freeze({
        ...
        kind: "unique",
        sdk_checks: "that a receipt is internally consistent, that it names a plan revision that hashes to its own contents, " +
            "and that it has not expired. It cannot know whether a receipt with this idempotency key already exists.",
        persistence_must: "hold a unique index on (tenant_id, idempotency_key) and return the existing receipt rather than writing a " +
        ...
        if_violated: "one act of confirming produces two receipts. Each one authorises a run, so a network retry doubles what " +
    ...
    Object.freeze({
        ...
        kind: "compare_and_set",
        sdk_checks: "that the latest plan revision the caller supplied is the revision the receipt names -- and it requires " +
        ...
        persistence_must: "re-read the latest plan revision and insert the receipt in one transaction, conditional on the stored " +
        ...
        if_violated: "the receipt authorises a revision that was superseded between the summary being rendered and the " +
    ...
    Object.freeze({
        ...
        kind: "unique",
        sdk_checks: "that an outcome names an authorization that hashes to its own contents and a capsule that answers that " +
            "same authorization. It cannot know whether another outcome already closed it.",
        persistence_must: "hold a unique index on authorization_ref. One piece of authorised work ends once, and the split in " +
            "task.ts is what makes that expressible: retries are attempts on the job, not second outcomes.",
        if_violated: "one authorization has two terminal answers -- succeeded and failed -- and both verify. Which one a reader " +
            "is shown decides whether the study reports a number or a refusal.",
    }),
]);
/** The invariant names, as immutable plain data. */
export const STUDY_PERSISTENCE_INVARIANT_NAMES = Object.freeze(STUDY_PERSISTENCE_INVARIANTS.map((invariant) => invariant.name));
