...
const resolutionRules = new Map(QUESTION_RESOLUTIONS.map((entry) => [entry.resolution, entry]));
export function questionResolutionRule(resolution) {
...
/** Still worth asking. */
export function isQuestionOutstanding(question) {
...
export function isSettledWithoutAnAnswer(question) {
 ...
 */
export const AnswerProvenanceSchema = z
    .object({
    ...
    .strict();
export const SPECIFICATION_QUESTION_TARGETS = Object.freeze([
...
const QUESTION_TARGET = new RegExp(`^(?:${SPECIFICATION_QUESTION_TARGETS.map((target) => target.indexable ? `${target.path}(?:\\[[0-9]{1,3}\\])?` : target.path).join("|")})$`);
/**
 ...
 */
export function questionTargetsField(question, path) {
    if (question.targets === path)
        return true;
...
export const OpenQuestionSchema = z
    ...
    if (!isChoice && question.allowed_choices !== null) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: `Choices are offered beside a ${question.answer_type} answer, which nothing constrains them to. ` +
    ...
    if (!outstanding && question.answer_provenance === null) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: `A question resolved ${question.resolution} carries a statement somebody made -- including "I looked and ` +
