import { z } from "zod";
import { GeneratedArtifactSchema } from "./artifact.js";
...
import { renderCellValue } from "./tables.js";
/**
...
export const FigureValueRefSchema = z
    .object({
    ...
    if (ref.kind === "NODE") {
        ...
        for (const key of ["table_id", "row_id", "column_id"]) {
            if (ref[key] === null)
                continue;
            context.addIssue({
                code: z.ZodIssueCode.custom,
                message: `A NODE reference reads from a node, so ${key} has nothing to address. Two ways of naming one ` +
    ...
    if (ref.node_hash !== null) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: "A TABLE_CELL reference reads through the table, which names the node. Naming both puts two answers in " +
...
export const FigurePointSchema = z
    .object({ x: FigureValueRefSchema, y: FigureValueRefSchema })
    .strict();
export const FigureSeriesSchema = z
    .object({
    ...
    .strict();
export const FigureAxisSchema = z
    .object({
    label: z.string().min(1).max(256),
    ...
    .strict();
export const FigureSpecSchema = z
    .object({
    ...
    .strict();
export const StudyFigureSchema = z
    .object({
 ...
 */
const PERMITTED_SVG_ELEMENTS = Object.freeze([
    "svg",
    "g",
    "defs",
    "title",
 ...
 */
const SVG_NAMESPACE_DECLARATION = /\sxmlns(?::[A-Za-z][A-Za-z0-9_.-]*)?\s*=\s*"http:\/\/www\.w3\.org\/2000\/svg"/g;
 ...
 */
export function sanitizeStudySvg(svg, path = studyPath("svg")) {
    const findings = [];
    if (SCRIPT_ELEMENT.test(svg)) {
        findings.push(finding("SVG_SCRIPT_REFUSED", path, "This SVG carries a script element. Script in a document rendered beside verified results runs with the " +
            "reader's authority over the page that is telling them what has been checked."));
    }
    if (JAVASCRIPT_URI.test(svg)) {
        findings.push(finding("SVG_SCRIPT_REFUSED", path, "This SVG carries a `javascript:` URI, which is a script element written as an attribute."));
    }
    if (NUMERIC_CHARACTER_REFERENCE.test(svg)) {
        findings.push(finding("SVG_SCRIPT_REFUSED", path, "This SVG carries a numeric character reference (`&#`). A parser resolves it and a scanner does not, " +
    ...
    if (FOREIGN_OBJECT.test(svg)) {
        findings.push(finding("SVG_FOREIGN_OBJECT_REFUSED", path, "This SVG embeds a foreign document (`foreignObject`, `iframe`, `embed` or `object`). Everything inside " +
    ...
    if (EXTERNAL_REFERENCE.test(withoutNamespace)) {
        findings.push(finding("SVG_EXTERNAL_REFERENCE_REFUSED", path, "This SVG references something outside itself. A figure that fetches at render time shows whatever the " +
    ...
    if (MARKUP_DECLARATION.test(svg)) {
        findings.push(finding("SVG_EXTERNAL_REFERENCE_REFUSED", path, "This SVG carries a markup declaration or processing instruction (`<!` or `<?`). An entity declaration is " +
    ...
    if (EVENT_HANDLER.test(svg)) {
        findings.push(finding("SVG_EVENT_HANDLER_REFUSED", path, "This SVG carries an event handler attribute. A handler is script that runs when a reader moves a mouse " +
            "over a figure."));
    }
    ...
    for (const match of svg.matchAll(ELEMENT_NAME)) {
        ...
        if (permittedSvgElements.has(local))
            continue;
        if (seenElements.has(local))
            continue;
        seenElements.add(local);
        findings.push(finding("SVG_ELEMENT_NOT_PERMITTED", path, `This SVG uses the element ${JSON.stringify(local)}, which is outside the subset this family stores. ` +
 ...
 * arrived in a list.
 */
export function indexTableCells(tables) {
    const index = new Map();
    for (const table of tables) {
        const valueColumns = new Set(table.columns.filter((column) => column.role === "VALUE").map((column) => column.column_id));
        for (const row of table.rows) {
            for (const cell of row.cells) {
                if (!valueColumns.has(cell.column_id) || cell.node_hash === null)
                    continue;
                const key = tableCellKey(table.table_id, row.row_id, cell.column_id);
                if (!index.has(key))
                    index.set(key, cell.node_hash);
            }
        }
    }
    return index;
...
export function resolveFigureValueRef(ref, cells) {
    if (ref.kind === "NODE")
        return ref.node_hash;
    if (ref.table_id === null || ref.row_id === null || ref.column_id === null)
        return null;
 ...
 */
export function figureFindings(figure, sources, cells, index, section = "figures") {
    const findings = [];
    const at = (...parts) => studyPath(section, index, ...parts);
    const seriesIds = new Set();
    figure.spec.series.forEach((series, seriesIndex) => {
        if (seriesIds.has(series.series_id)) {
            findings.push(finding("REPORT_DUPLICATE_ID", at("spec", "series", seriesIndex, "series_id"), `Two series in this figure share the id ${JSON.stringify(series.series_id)}, so a legend has two ` +
        ...
        series.points.forEach((point, pointIndex) => {
            for (const axis of ["x", "y"]) {
                ...
                if (nodeHash === null) {
                    findings.push(finding("FIGURE_POINT_UNRESOLVED", where, ref.kind === "NODE"
                ...
                if (source === undefined) {
                    findings.push(finding("FIGURE_POINT_UNRESOLVED", where, `This coordinate reads node ${nodeHash}, and the package does not carry it. A chart drawn from ` +
                ...
                if (source.quantity !== null)
                    continue;
                findings.push(finding("FIGURE_POINT_UNRESOLVED", where, `This coordinate reads the ${source.kind} node ${JSON.stringify(source.label)}, which carries no ` +
    ...
    if (figure.svg_artifact !== null) {
        ...
        const overSize = limitFinding(at("svg_artifact", "byte_size"), "declared SVG bytes", Number.isFinite(declared) ? declared : STUDY_PACKAGE_LIMITS.max_svg_bytes + 1, STUDY_PACKAGE_LIMITS.max_svg_bytes);
        if (overSize !== null)
...
export function figureListFindings(figures, section = "figures") {
    const findings = [];
    const seen = new Set();
    figures.forEach((figure, index) => {
        if (seen.has(figure.figure_id)) {
            findings.push(finding("REPORT_DUPLICATE_ID", studyPath(section, index, "figure_id"), `Two figures share the id ${JSON.stringify(figure.figure_id)}. A report segment naming it has two ` +
                "figures to render."));
        }
 ...
 * unmeasured.
 */
function plottableValue(ref, cells, sources) {
    const nodeHash = resolveFigureValueRef(ref, cells);
    if (nodeHash === null)
        return null;
    const source = sources.get(nodeHash);
    if (source === undefined || source.quantity === null)
        return null;
 ...
 */
export function renderFigureSvg(figure, sources, cells) {
    ...
    const toX = (value) => MARGIN.left + ((value - xMin) / xSpan) * plotWidth;
    const toY = (value) => MARGIN.top + plotHeight - ((value - yMin) / ySpan) * plotHeight;
    const parts = [];
    parts.push(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${WIDTH} ${HEIGHT}" role="img" ` +
    ...
    parts.push(`<line x1="${coordinate(MARGIN.left)}" y1="${coordinate(MARGIN.top + plotHeight)}" ` +
        `x2="${coordinate(MARGIN.left + plotWidth)}" y2="${coordinate(MARGIN.top + plotHeight)}" ` +
        `stroke="currentColor" stroke-width="1"/>`);
    parts.push(`<line x1="${coordinate(MARGIN.left)}" y1="${coordinate(MARGIN.top)}" ` +
        `x2="${coordinate(MARGIN.left)}" y2="${coordinate(MARGIN.top + plotHeight)}" ` +
        `stroke="currentColor" stroke-width="1"/>`);
    const axisLabel = (axis) => axis.unit === null ? axis.label : `${axis.label} (${axis.unit})`;
    parts.push(`<text x="${coordinate(MARGIN.left + plotWidth / 2)}" y="${coordinate(HEIGHT - 16)}" ` +
        `text-anchor="middle" font-size="12">${escapeSvgText(axisLabel(figure.spec.x_axis))}</text>`);
    parts.push(`<text x="16" y="${coordinate(MARGIN.top + plotHeight / 2)}" text-anchor="middle" font-size="12" ` +
    ...
    for (const entry of plotted) {
        if (figure.spec.kind === "LINE") {
            const points = entry.points
                ...
                .join(" ");
            if (points !== "") {
                parts.push(`<polyline fill="none" stroke="currentColor" stroke-width="1.5" points="${points}"/>`);
            }
            continue;
        }
        for (const point of entry.points) {
            if (figure.spec.kind === "SCATTER") {
                parts.push(`<circle cx="${coordinate(toX(point.x))}" cy="${coordinate(toY(point.y))}" r="3" ` +
                    `fill="currentColor"/>`);
                continue;
            }
            const top = toY(point.y);
            const base = MARGIN.top + plotHeight;
            parts.push(`<rect x="${coordinate(toX(point.x) - barWidth / 2)}" y="${coordinate(Math.min(top, base))}" ` +
                `width="${coordinate(barWidth)}" height="${coordinate(Math.abs(base - top))}" ` +
                `fill="currentColor"/>`);
        }
        parts.push(`<desc>${escapeSvgText(`${entry.series.label}: ${entry.points.length} plotted points`)}</desc>`);
    }
    parts.push("</svg>");
    return parts.join("");
}
 ...
 * from the same node.
 */
export function renderFigureSummary(figure, sources, cells) {
