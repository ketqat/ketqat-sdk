...
import { AttestationLevelSchema, ContentHashSchema, ExecutionResourceClassSchema, RevisionRefSchema, STUDY_SCHEMA_VERSION, } from "./common.js";
...
export const AuthorizationScopeSchema = z.string().regex(AUTHORIZATION_SCOPE, {
    message: "An authorization scope is `resource:action` in lowercase, letters, digits and underscores. Two spellings of " +
...
export const ConfirmationNonceSchema = z.string().regex(/^[0-9a-f]{32,128}$/, {
    message: "A confirmation nonce is 32 to 128 lowercase hex digits: at least 128 bits, one spelling. It is what makes " +
...
export const IdempotencyKeySchema = z.string().regex(/^[A-Za-z0-9_-]{16,128}$/, {
    message: "An idempotency key is 16 to 128 characters of unreserved URL-safe text. It is the client's statement that a " +
 ...
 * caption correctly.
 */
export const CONFIRMATION_RECEIPT_LIMITATIONS = Object.freeze([
...
export const ConfirmationReceiptSchema = z
    ...
    if (Date.parse(receipt.expires_at) <= Date.parse(receipt.confirmed_at)) {
        context.addIssue({
            ...
            message: `A receipt confirmed at ${receipt.confirmed_at} and expiring at ${receipt.expires_at} authorises ` +
    ...
    if (receipt.estimated_credits > receipt.max_credits) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: `The plan expects ${receipt.estimated_credits} credits and the ceiling is ${receipt.max_credits}. ` +
 ...
 */
export function buildConfirmationReceipt(input) {
    const subject = `study plan revision ${input.plan.revision}`;
    const confirmation = verifyPlanConfirmation(input.plan, input.plan.content_hash, input.latestPlanRevision);
    if (!confirmation.ok)
        return confirmation;
    if (input.estimatedCredits > input.plan.max_credits) {
        return {
            ...
            refusal: {
                ...
                message: `The estimate shown to the actor is ${input.estimatedCredits} credits and the plan's ceiling is ` +
 ...
 */
export function verifyConfirmationReceipt(receipt, plan, check = {}) {
    ...
    if (receiptHashNow !== receipt.content_hash) {
        return {
            ...
            refusal: {
                ...
                message: `The receipt claims hash ${receipt.content_hash} and its own contents canonicalize to ` +
    ...
    if (planHashNow !== plan.content_hash) {
        return {
            ...
            refusal: {
                ...
                message: `The plan claims hash ${plan.content_hash} and its own contents canonicalize to ${planHashNow}. ` +
    ...
    if (receipt.plan_ref.revision_hash !== planHashNow) {
        return {
            ...
            refusal: {
                ...
                message: `The receipt authorises revision ${receipt.plan_ref.revision} ` +
    ...
    if (receipt.plan_semantic_hash !== planSemanticNow) {
        return {
            ...
            refusal: {
                subject,
                code: "CONFIRMATION_HASH_MISMATCH",
                message: `The receipt records the plan's semantic hash as ${receipt.plan_semantic_hash} and the plan's own ` +
    ...
    if (receipt.study_ref !== plan.study_ref) {
        return {
            ...
            refusal: {
                ...
                message: `The receipt belongs to study ${receipt.study_ref} and the plan to ${plan.study_ref}. An approval given ` +
    ...
    if (check.latestPlanRevision != null && check.latestPlanRevision.revision_hash !== planHashNow) {
        return {
            ...
            refusal: {
                subject,
                code: "PLAN_REVISION_SUPERSEDED",
                message: `Revision ${check.latestPlanRevision.revision} (${check.latestPlanRevision.revision_hash}) is now the ` +
    ...
    if (check.at !== undefined && Date.parse(check.at) >= Date.parse(receipt.expires_at)) {
        return {
            ...
            refusal: {
                ...
                message: `The receipt expired at ${receipt.expires_at} and the run would start at ${check.at}. An approval has a ` +
 ...
 * scope by accident.
 */
export function receiptGrantsScope(receipt, scope) {
