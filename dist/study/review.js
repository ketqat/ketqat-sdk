...
export const ReproductionRecordSchema = z
    .object({
    schema_version: z.string().min(1),
    hash_rules_id: z.literal(STUDY_HASH_RULES_ID),
    study_ref: StudyIdSchema,
    ...
    if (record.outcome !== "INCONCLUSIVE" && record.observed_node_hash === null) {
        context.addIssue({
            ...
            message: `A ${record.outcome} outcome is a comparison between two results, so the reproduction's own result must ` +
    ...
    if (record.observed_node_hash === record.original_node_hash) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: "The reproduction names the original as its own result. Identity in this graph is the content hash, so " +
...
export function reviewsOf(reviews, nodeHash) {
...
export function reproductionsOf(reproductions, nodeHash) {
