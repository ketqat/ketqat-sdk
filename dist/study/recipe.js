...
import { ResourceLimitsSchema } from "./capsule.js";
import { finding, studyPath } from "./findings.js";
export const APPROVED_RUNNERS = Object.freeze([
    Object.freeze({
...
export const APPROVED_RUNNER_NAMES = Object.freeze(APPROVED_RUNNERS.map((entry) => entry.runner));
/** The working lookup, module-private and built from the frozen tuple. */
const approvedRunners = new Set(APPROVED_RUNNER_NAMES);
export function isApprovedRunner(value) {
...
export const PlatformRequirementSchema = z
    ...
    .strict();
export const ReproductionRecipeSchema = z
    .object({
    /**
    ...
    container_digest: z.string().regex(CONTAINER_DIGEST, {
        message: "A container pin is an image digest, `sha256:` and 64 lowercase hex characters. A tag is not a pin: it " +
     ...
     */
    argv: z
        .array(z.string().min(1).max(1024).regex(SAFE_ARGV_ELEMENT, {
        message: "An argv element carries only characters no shell treats specially. An argument that needs a space " +
    ...
    environment_allowlist: z
        ...
        message: "An environment allowlist carries variable names, never values: uppercase, digits and underscores, " +
    ...
    if (recipe.network_policy === "NONE" && recipe.allowed_hosts.length > 0) {
        context.addIssue({
            ...
            message: "The recipe declares that the run takes no network and then names hosts it may reach. Two readings of " +
    ...
    if (recipe.network_policy === "ALLOWLIST" && recipe.allowed_hosts.length === 0) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: "An allowlist policy with an empty list permits nothing, which is what NONE says, and says it in a way " +
 ...
 */
export function renderRecipeCommand(recipe) {
 ...
 */
export function recipeFindings(recipe, carriedArtifactHashes, section = "recipe") {
    const findings = [];
    if (!isApprovedRunner(recipe.runner)) {
        findings.push(finding("RECIPE_RUNNER_NOT_APPROVED", studyPath(section, "runner"), `This build does not approve ${JSON.stringify(recipe.runner)} for automatic execution. Approved runners: ` +
    ...
    for (const [key, refs] of sections) {
        refs.forEach((ref, index) => {
            ...
            if (seenNames.has(scoped)) {
                findings.push(finding("RECIPE_ARTIFACT_UNRESOLVED", studyPath(section, key, index, "name"), `The recipe names ${JSON.stringify(ref.name)} twice in ${key}. Two entries under one name are two ` +
            ...
            // of a hardware run.
            if (ref.resolution.kind !== "INLINE_IN_BUNDLE")
                return;
            if (carriedArtifactHashes.has(ref.content_hash))
                return;
            findings.push(finding("RECIPE_ARTIFACT_UNRESOLVED", studyPath(section, key, index, "content_hash"), `The recipe says ${JSON.stringify(ref.name)} travels inside this package, and no artifact in it has ` +
