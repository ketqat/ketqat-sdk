...
export const CheckToolSchema = z
    .object({
    ...
    .strict();
export const CheckLedgerEntrySchema = z
    .object({
    ...
    if (entry.status !== "PASS" && entry.reason.trim() === "") {
        context.addIssue({
            ...
            message: `A ${entry.status} check must say why. A status with no reason records that something is wrong, or was ` +
    ...
    if (entry.status === "NOT_RUN" && entry.output_ref !== null) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: "A check that did not run produced nothing, so there is no output for this hash to name. An output " +
 ...
 */
export function checkLedgerSummary(entries) {
    let passed = 0;
    let failed = 0;
    let notRun = 0;
    let inconclusive = 0;
    ...
    for (const entry of entries) {
        if (entry.status === "PASS")
            ...
            inconclusive += 1;
        if (entry.requirement === "REQUIRED" && entry.status !== "PASS")
 ...
 */
export function checkLedgerFindings(entries, section = "check_ledger") {
    const findings = [];
    const seen = new Map();
    entries.forEach((entry, index) => {
        const first = seen.get(entry.check_id);
        if (first !== undefined) {
            findings.push(finding("CHECK_LEDGER_DUPLICATE_ID", studyPath(section, index, "check_id"), `Check ${JSON.stringify(entry.check_id)} is recorded twice, first at index ${first}. A ledger with two ` +
 ...
 */
export function absentRequiredChecks(entries, required, section = "check_ledger") {
    const present = new Set(entries.map((entry) => entry.check_id));
    return required
        .filter((id) => !present.has(id))
        .map((id) => finding("CHECK_LEDGER_REQUIRED_CHECK_ABSENT", studyPath(section), `The check ${JSON.stringify(id)} is required here and the ledger does not mention it. An absent check is ` +
