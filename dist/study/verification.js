import { z } from "zod";
import { AttestationLevelSchema } from "./common.js";
/**
 ...
 * control.
 */
export function deriveStudyVerificationStatus(levels) {
    if (!levels.schema_valid || !levels.canonicalizable)
        return "REFUSED";
    const structural = levels.hash_matches &&
 ...
 */
export function notEstablished(levels) {
    const sentences = [
    ...
    if (levels.hash_matches) {
        sentences.push("A matching hash does not mean the content is correct. A wrong number, honestly recorded and correctly " +
            "hashed, verifies exactly like a right one.");
    }
    if (!levels.science_recomputed) {
        sentences.push("No model was re-run. The estimates and decisions this package quotes were not rebuilt from their inputs " +
            "here, so a conclusion that does not follow from them would not have been caught.");
    }
    if (!levels.independent_reproduction_present) {
        sentences.push("No reproduction is recorded. Nothing in this package reports a second run of the work reaching the same " +
            "result.");
    }
    else {
        sentences.push("A reproduction record reports a match. Whether the party that ran it was independent of the party that " +
    ...
    sentences.push("A verified chain is not a complete one. Reordering and splicing are detectable; truncation is not, without " +
 ...
 */
export function refusedStudyVerificationLevels() {
    return {
        schema_valid: false,
