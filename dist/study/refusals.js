 ...
 * change that.
 */
export function studyRulesIdRefusal(subject, record) {
    ...
    if (typeof recorded !== "string" || recorded.length === 0) {
        return {
            ...
            message: `A study-family record must name its hash rules in ${STUDY_HASH_RULES_KEY}; nothing is inferred from ` +
    ...
    if (!STUDY_KNOWN_HASH_RULES_IDS.includes(recorded)) {
        return {
            ...
            message: `This build does not know the hash rules id ${JSON.stringify(recorded)}. Known ids: ` +
 ...
 */
export function studyNotHashableRefusal(subject, error) {
    if (!(error instanceof StudyHashRefusalError))
        throw error;
    return {
        subject,
