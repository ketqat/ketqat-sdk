...
import { artifactRefListSchema } from "./artifact.js";
import { CancellationSchema } from "./capsule.js";
import { ContentHashSchema, ExecutionResourceClassSchema, RevisionRefSchema, STUDY_SCHEMA_VERSION, } from "./common.js";
import { studySelfHash } from "./hash.js";
import { StudyIdSchema } from "./identity.js";
import { receiptGrantsScope, verifyConfirmationReceipt, } from "./receipt.js";
...
export const STUDY_OPERATION_SCOPES = Object.freeze([
    Object.freeze({ operation: "STUDY_BASELINE_RUN", scope: "study:execute" }),
    Object.freeze({ operation: "STUDY_RESOURCE_ESTIMATE", scope: "study:estimate" }),
    Object.freeze({ operation: "STUDY_BENCHMARK_RUN", scope: "study:execute" }),
    Object.freeze({ operation: "STUDY_REPRODUCTION", scope: "study:execute" }),
]);
/** The working lookup, module-private and built from the frozen tuple. */
const scopeByOperation = new Map(STUDY_OPERATION_SCOPES.map((entry) => [entry.operation, entry.scope]));
...
export const ResourceCeilingSchema = z
    .object({
    ...
    .strict();
export const StudyTaskAuthorizationSchema = z
    .object({
     ...
     */
    input_refs: artifactRefListSchema("authorised input"),
 ...
 */
export function authoriseStudyTask(input) {
    ...
    if (required === null) {
        return {
            ...
            refusal: {
                ...
                message: `This build declares no scope for operation ${input.requestedOperation}, so nothing can say whether the ` +
    ...
    if (!receiptGrantsScope(input.receipt, required)) {
        return {
            ...
            refusal: {
                subject,
                code: "CONFIRMATION_SCOPE_INSUFFICIENT",
                message: `A ${input.requestedOperation} needs the scope ${required}, and the confirmation carries ` +
...
export const STUDY_EXECUTION_JOB_TERMINAL_STATUSES = Object.freeze([
    "SUCCEEDED",
    "FAILED",
    "CANCELLED",
]);
export const StudyExecutionJobProgressSchema = z
    .object({
    ...
    if (progress.total_units !== null && progress.completed_units > progress.total_units) {
        context.addIssue({
            ...
            message: `${progress.completed_units} of ${progress.total_units} units is not progress, it is a counter that has ` +
                "outrun its own total. A reader shown 130% concludes the total was wrong and stops believing the figure.",
            path: ["completed_units"],
        });
    }
});
export const StudyExecutionJobSchema = z
    .object({
    schema_version: z.string().min(1),
    /**
    ...
    if (job.attempt > job.max_attempts) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: `Attempt ${job.attempt} of a job allowed ${job.max_attempts} is a retry past the limit that was set for ` +
    ...
    if (job.status === "RETRY_SCHEDULED" && job.attempt >= job.max_attempts) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: `A retry is scheduled on attempt ${job.attempt} of ${job.max_attempts}, and there is no attempt left ` +
                "to schedule. A job parked in this state never moves, and a reader waiting on it is waiting for " +
                "something that will not happen.",
            path: ["status"],
        });
    }
    if (STUDY_EXECUTION_JOB_TERMINAL_STATUSES.includes(job.status) &&
        job.progress.total_units !== null &&
        job.status === "SUCCEEDED" &&
        job.progress.completed_units !== job.progress.total_units) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: `A job that succeeded reports ${job.progress.completed_units} of ${job.progress.total_units} units ` +
    ...
    if (job.cancellation.cancelled && job.status !== "CANCELLED") {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: `A cancelled job is at ${job.status}. The flag and the status are two statements about one thing, and a ` +
...
export const TaskOutcomeSchema = z
    ...
    if (outcome.terminal_status === "SUCCEEDED" && outcome.capsule_ref === null) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: "A successful run produced a capsule, and this outcome names none. A success with nothing behind it is " +
    ...
    if (outcome.terminal_status !== "SUCCEEDED" && outcome.reason === null) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: `A ${outcome.terminal_status} outcome must say why. This is the record a reader goes to when they ask ` +
    ...
    if (outcome.terminal_status === "SUCCEEDED" && outcome.reason !== null) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: "A successful run has no reason to explain. A note recorded beside SUCCEEDED reads as a caveat on a " +
    ...
    if (authorizationHashNow !== input.authorization.content_hash) {
        return {
            ...
            refusal: {
                ...
                message: `The authorization claims hash ${input.authorization.content_hash} and its own contents canonicalize ` +
    ...
    if (capsule !== null && capsule.authorization_ref !== input.authorization.content_hash) {
        return {
            ...
            refusal: {
                ...
                message: `The capsule answers authorization ${capsule.authorization_ref} and this outcome closes ` +
 ...
 */
export const TASK_CHAIN_DOES_NOT_ESTABLISH = Object.freeze([
...
export function verifyTaskAuthorizationChain(input) {
    const problems = [];
    ...
    if (authorizationHashNow !== authorization.content_hash) {
        add("REVISION_BASE_EDITED", `The authorization claims hash ${authorization.content_hash} and its own contents canonicalize to ` +
    ...
    if (!confirmation.ok)
        problems.push(confirmation.refusal);
    if (authorization.confirmation_receipt_ref !== receipt.content_hash) {
        add("TASK_REFERENCE_UNRESOLVED", `The authorization was granted under receipt ${authorization.confirmation_receipt_ref} and the receipt in ` +
            `hand is ${receipt.content_hash}. One of the two is a different approval, and which one decides what was ` +
            "actually permitted.");
    }
    if (authorization.plan_ref.revision_hash !== receipt.plan_ref.revision_hash ||
        authorization.plan_ref.revision !== receipt.plan_ref.revision) {
        add("PLAN_REVISION_SUPERSEDED", `The authorization is bound to plan revision ${authorization.plan_ref.revision} ` +
    ...
    for (const [label, value] of [
        ...
        if (value === study.study_id)
            continue;
        add("TASK_REFERENCE_UNRESOLVED", `${label} belongs to study ${value} and the study in hand is ${study.study_id}. An approval given inside ` +
    ...
    if (required === null || !receiptGrantsScope(receipt, required)) {
        add("CONFIRMATION_SCOPE_INSUFFICIENT", `A ${authorization.requested_operation} needs the scope ${required ?? "(none declared)"}, and the receipt ` +
            `carries ${receipt.authorization_scope.join(", ")}.`);
    }
    if (authorization.resource_ceiling.max_credits > receipt.max_credits) {
        add("CREDITS_MAXIMUM_EXCEEDED", `The authorization allows ${authorization.resource_ceiling.max_credits} credits and the confirmation ` +
    ...
    if (authorization.resource_ceiling.resource_class !== receipt.resource_class) {
        add("EXECUTION_CLASS_MISMATCH", `The authorization is for ${authorization.resource_ceiling.resource_class} and the confirmation was given ` +
    ...
    if (capsule !== null) {
        if (capsule.authorization_ref !== authorization.content_hash) {
            add("TASK_REFERENCE_UNRESOLVED", `The capsule answers authorization ${capsule.authorization_ref} and the authorization in hand is ` +
                `${authorization.content_hash}.`);
        }
        if (capsule.execution.kind !== authorization.resource_ceiling.resource_class) {
            add("EXECUTION_CLASS_MISMATCH", `The run executed as ${capsule.execution.kind} and the authorization permits ` +
        ...
        if (capsule.execution.kind === "HARDWARE" &&
            ...
            add("TASK_REFERENCE_UNRESOLVED", `The hardware capsule was submitted under receipt ${capsule.execution.confirmation_receipt_ref} and the ` +
        ...
        if (capsule.execution.kind === "HARDWARE" &&
            ...
            add("CREDITS_MAXIMUM_EXCEEDED", `The capsule records an authorised maximum of ${capsule.execution.cost_confirmation.authorized_maximum} ` +
    ...
    if (outcome !== null) {
        if (outcome.authorization_ref !== authorization.content_hash) {
            add("TASK_REFERENCE_UNRESOLVED", `The outcome closes authorization ${outcome.authorization_ref} and the authorization in hand is ` +
                `${authorization.content_hash}.`);
        }
        ...
        if (outcome.capsule_ref !== null && outcome.capsule_ref !== capsuleHash) {
            add("TASK_REFERENCE_UNRESOLVED", `The outcome names capsule ${outcome.capsule_ref} and the capsule in hand is ` +
