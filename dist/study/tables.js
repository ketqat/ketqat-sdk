...
export const TableCellSchema = z
    ...
    if (cell.text === null && cell.node_hash === null) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: "A cell carries either text or a node, and this one carries neither. An empty cell renders as a blank a " +
    ...
    if (cell.text !== null && cell.node_hash !== null) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: "A cell carries text or a node, never both. Two statements in one cell are free to disagree, and the " +
...
export const TableRowSchema = z
    ...
    .strict();
export const StudyTableSchema = z
    .object({
    ...
    if (table.role === "RESULTS" && !table.columns.some((column) => column.role === "VALUE")) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: "A results table declares no value column, so it reports no results. A table of labels is an inventory; " +
 ...
 */
export function renderCellValue(quantity) {
    if (quantity === null)
        return "UNKNOWN";
...
function cellsByColumn(row) {
    ...
    for (const cell of row.cells) {
        if (!index.has(cell.column_id))
            index.set(cell.column_id, cell);
    }
    return index;
}
/**
 ...
 */
export function renderTableCsv(table, sources) {
    const lines = [csvHeaderFields(table).map(csvField).join(",")];
    for (const row of table.rows) {
        ...
        for (const column of table.columns) {
            ...
            if (cell === undefined) {
                throw new StudyTableRenderError(`Table ${table.table_id} row ${row.row_id} has no cell for column ${column.column_id}. Check the table ` +
            ...
            if (nodeHash === null || source === undefined) {
                throw new StudyTableRenderError(`Table ${table.table_id} row ${row.row_id} reads column ${column.column_id} from a node the package ` +
...
export function tableCsvBytes(table, sources) {
 ...
 */
export function tableCsvArtifact(table, sources, schemaVersion) {
...
export function renderTableMarkdown(table, sources) {
    ...
    for (const row of table.rows) {
        ...
        for (const column of table.columns) {
            const cell = index.get(column.column_id);
            if (cell === undefined) {
                throw new StudyTableRenderError(`Table ${table.table_id} row ${row.row_id} has no cell for column ${column.column_id}.`);
            }
            if (column.role === "LABEL") {
                fields.push(cell.text ?? "");
                continue;
 ...
 */
export function tableFindings(table, sources, index, section = "tables") {
    const findings = [];
    const at = (...parts) => studyPath(section, index, ...parts);
    const columns = new Map();
    table.columns.forEach((column, columnIndex) => {
        if (columns.has(column.column_id)) {
            findings.push(finding("TABLE_SHAPE_MISMATCH", at("columns", columnIndex, "column_id"), `Column ${JSON.stringify(column.column_id)} is declared twice. A cell naming it has two columns to land ` +
    ...
    table.rows.forEach((row, rowIndex) => {
        if (rowIds.has(row.row_id)) {
            findings.push(finding("REPORT_DUPLICATE_ID", at("rows", rowIndex, "row_id"), `Row id ${JSON.stringify(row.row_id)} appears twice in this table, so a figure naming it has two rows ` +
        ...
        row.cells.forEach((cell, cellIndex) => {
            ...
            if (column === undefined) {
                findings.push(finding("TABLE_SHAPE_MISMATCH", at("rows", rowIndex, "cells", cellIndex, "column_id"), `This cell names column ${JSON.stringify(cell.column_id)}, which the table does not declare. A cell ` +
            ...
            if (seen.has(cell.column_id)) {
                findings.push(finding("TABLE_SHAPE_MISMATCH", at("rows", rowIndex, "cells", cellIndex, "column_id"), `This row has two cells for column ${JSON.stringify(cell.column_id)}. One row, one column, one value.`));
                return;
            }
            ...
            if (column.role === "LABEL") {
                if (cell.text !== null)
                    return;
                findings.push(finding("TABLE_SHAPE_MISMATCH", at("rows", rowIndex, "cells", cellIndex, "text"), `Column ${JSON.stringify(column.column_id)} is a label column and this cell carries a node instead of ` +
                    "text. A node in a label column renders as whatever the renderer decides to show of it."));
                return;
            }
            if (cell.node_hash === null) {
                findings.push(finding("TABLE_CELL_WITHOUT_NODE", at("rows", rowIndex, "cells", cellIndex, "node_hash"), `Column ${JSON.stringify(column.column_id)} is a value column and this cell names no node. A number ` +
            ...
            if (source === undefined) {
                findings.push(finding("EVIDENCE_NODE_UNRESOLVED", at("rows", rowIndex, "cells", cellIndex, "node_hash"), `This cell reads node ${cell.node_hash}, and the package does not carry it. The cell renders as a ` +
                    "number like any other, and a reader has no way to discover that it stands alone."));
                return;
            }
            if (source.quantity === null) {
                findings.push(finding("RESULT_ROW_WITHOUT_VALUE", at("rows", rowIndex, "cells", cellIndex, "node_hash"), `This cell reads its value from the ${source.kind} node ${JSON.stringify(source.label)}, which carries ` +
            ...
            if (column.unit !== null && source.quantity.unit !== column.unit) {
                findings.push(finding("TABLE_SHAPE_MISMATCH", at("rows", rowIndex, "cells", cellIndex, "node_hash"), `The column is in ${JSON.stringify(column.unit)} and this node's quantity is in ` +
        ...
        for (const column of table.columns) {
            if (seen.has(column.column_id))
                continue;
            findings.push(finding("TABLE_SHAPE_MISMATCH", at("rows", rowIndex, "cells"), `This row has no cell for column ${JSON.stringify(column.column_id)}. A missing cell renders as a blank, ` +
 ...
 */
export function tableCsvArtifactFindings(table, sources, schemaVersion, index, section = "tables") {
    const findings = [];
    const at = (...parts) => studyPath(section, index, ...parts);
    const bytes = tableCsvBytes(table, sources);
    const overSize = limitFinding(at("csv_artifact", "byte_size"), "CSV bytes", bytes.length, STUDY_PACKAGE_LIMITS.max_csv_bytes);
    if (overSize !== null)
        findings.push(overSize);
    if (String(bytes.length) !== table.csv_artifact.byte_size) {
        findings.push(finding("TABLE_CSV_ARTIFACT_MISMATCH", at("csv_artifact", "byte_size"), `The table renders to ${bytes.length} bytes and the artifact records ${table.csv_artifact.byte_size}. ` +
    ...
    if (expected !== table.csv_artifact.content_hash) {
        findings.push(finding("TABLE_CSV_ARTIFACT_MISMATCH", at("csv_artifact", "content_hash"), `The table renders to a CSV whose bytes hash to ${expected}, and the artifact claims ` +
 ...
 */
export function tableListFindings(tables, section = "tables") {
    const findings = [];
    const seen = new Set();
    tables.forEach((table, index) => {
        if (seen.has(table.table_id)) {
            findings.push(finding("REPORT_DUPLICATE_ID", studyPath(section, index, "table_id"), `Two tables share the id ${JSON.stringify(table.table_id)}. A report segment naming it has two tables ` +
        ...
        const overCaption = limitFinding(studyPath(section, index, "caption"), "caption bytes", captionBytes, STUDY_PACKAGE_LIMITS.max_report_bytes);
        if (overCaption !== null)
            findings.push(overCaption);
    });
    return findings;
}
