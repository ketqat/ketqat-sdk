 ...
 * long-lived study.
 */
export const DataRetentionSchema = z
    .object({
    ...
    if (!needsDays && retention.days !== null) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: `A period is stated beside ${retention.kind}, which does not delete on one. A reader would take the ` +
...
export const AllowedEgressSchema = z
    .object({
 ...
 */
export const DeletionPolicySchema = z
    .object({
...
export const DataHandlingPolicySchema = z
    ...
    if (policy.third_party_transfer === "NONE" && reachesOutward) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: "The policy permits egress to a third party and states that nothing is transferred to one. Egress is " +
    ...
    if (policy.pii_handling === "PRESENT_RESTRICTED" && policy.model_training_use !== "FORBIDDEN") {
        context.addIssue({
            code: z.ZodIssueCode.custom,
            message: "Personal data is present under restricted access and the policy permits training on it. Restricted " +
                "access is not a property a trained model keeps.",
            path: ["model_training_use"],
        });
    }
});
const clauses = (field, options, table) => {
...
function clauseFor(lookup, member) {
 ...
 */
export function dataHandlingSummary(policy) {
    ...
    return [
        ...
        clauseFor(DELETION_CLAUSES, policy.deletion_policy.kind).replace("{days}", String(policy.deletion_policy.within_days ?? "")),
