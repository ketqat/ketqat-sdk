...
import { HASH_VERSION_KEY, IDENTITY_KEYS, TIMING_KEYS, canonicalJsonForExcludedKeys, } from "../reproducibility/index.js";
/**
 ...
 * one cast.
 */
function frozenKeySet(name, keys) {
    const set = new Set(keys);
    for (const method of ["add", "delete", "clear"]) {
        Object.defineProperty(set, method, {
            value: () => {
                throw new TypeError(`${name} is frozen: ${method} would change the rules every study-v1 digest was computed under, and ` +
 ...
 * drifts.
 */
export const STUDY_EXCLUDED_KEYS = frozenKeySet("STUDY_EXCLUDED_KEYS", [
    ...IDENTITY_KEYS,
 ...
 */
const excludedByRulesId = new Map([
    [STUDY_HASH_RULES_ID, STUDY_EXCLUDED_KEYS],
]);
function excludedFor(rulesId) {
    const excluded = excludedByRulesId.get(rulesId);
    if (!excluded) {
        throw new Error(`Unknown study hash rules id ${rulesId}. Known ids: ${[...excludedByRulesId.keys()].join(", ")}.`);
    }
    return excluded;
}
/**
 ...
 */
export function studyRulesIdOf(input) {
    const recorded = input[STUDY_HASH_RULES_KEY];
    if (typeof recorded !== "string" || recorded.length === 0) {
        throw new Error("A study-family record must name its hash rules id explicitly; nothing is inferred. " +
 ...
 */
function isEmbeddedRecord(value) {
 ...
 */
const EMBEDDED_RECORD_EXEMPT_KEYS = frozenKeySet("EMBEDDED_RECORD_EXEMPT_KEYS", [
    STUDY_HASH_RULES_KEY,
    "content_hash",
    "created_at",
]);
/** No exemption at all: the ordinary case, where every excluded name is refused. */
const NO_EXEMPT_KEYS = frozenKeySet("NO_EXEMPT_KEYS", []);
/**
 ...
 * has one such key.
 */
function findNestedExcludedKey(value, path, atRecordRoot, excluded) {
    ...
    for (const key of Object.keys(source)) {
        ...
        if (exempt !== null && excluded.has(key) && !exempt.has(key))
            return here;
 ...
 */
export function assertNoNestedExcludedKeys(input, rulesId = studyRulesIdOf(input)) {
    const offending = findNestedExcludedKey(input, "", true, excludedFor(rulesId));
    if (offending === null)
        return;
    const name = offending.slice(offending.lastIndexOf(".") + 1);
    throw new Error(`A study record must not carry an excluded key below its own top level: ${offending} is named ` +
 ...
 */
function describeUnrepresentable(value, path) {
    if (typeof value === "number") {
        ...
        if (!Number.isFinite(value)) {
            return (`${path} is ${String(value)}, which is not a finite number. JavaScript canonicalizes ` +
        ...
        if (!Number.isInteger(value) || Math.abs(value) <= Number.MAX_SAFE_INTEGER)
            return null;
        return (`${path} is ${JSON.stringify(value)}, an integer outside ±Number.MAX_SAFE_INTEGER ` +
    ...
    if (typeof value === "string" && UNPAIRED_SURROGATE.test(value)) {
        return (`${path} contains an unpaired UTF-16 surrogate. Half a character is not a character: JavaScript escapes ` +
 ...
 */
function findUnrepresentableValue(value, path, excluded) {
    if (Array.isArray(value)) {
        for (let index = 0; index < value.length; index += 1) {
 ...
 */
export function assertNoUnrepresentableValues(input, rulesId = studyRulesIdOf(input)) {
 ...
 */
export function canonicalStudyJson(input, rulesId = studyRulesIdOf(input)) {
...
export function calculateStudyHash(input, rulesId = studyRulesIdOf(input)) {
 ...
 */
export function verifyStudyRecordHash(input) {
    ...
    if (typeof content === "string" && typeof reproducibility === "string") {
        throw new Error("A study record must carry one self-hash field, and this one carries both content_hash and " +
