...
export function distilledError(inputError) {
 ...
 * worse than refusing.
 */
export function requiredLevels(inputError, targetError) {
    ...
    if (inputError >= fixedPoint) {
        return {
            ...
            reason: `An input error of ${inputError} is at or above the 15-to-1 fixed point of ` +
 ...
 */
export function estimateFactoryCost(magicStates, logicalQubits, options = {}) {
    ...
    const perPatch = (distance) => 2 * distance * distance;
    const algorithmPhysical = logicalQubits * perPatch(algorithmDistance);
    const factoryPhysical = levels.levels === 0 ? 0 : levels.levels * STATES_PER_BLOCK * perPatch(factoryDistance);
    ...
    if (share > 0.5) {
        warnings.push(`The factory is ${(share * 100).toFixed(0)}% of the device -- larger than the algorithm it feeds. ` +
            "A T count reported without this footprint understates the hardware requirement by more than a " +
            "factor of two here.");
    }
    if (magicStates === 0) {
        warnings.push("This algorithm consumes no magic states, so no factory is needed. A Clifford circuit needs no " +
            "distillation at all.");
    }
    return {
