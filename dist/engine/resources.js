...
export function estimateResources(circuit, profile) {
    const tally = emptyTally();
    const couplingErrors = new Map();
    const couplingDurations = new Map();
    if (profile) {
        for (const coupling of profile.couplings) {
            const key = [coupling.control, coupling.target].sort((a, b) => a - b).join("-");
            if (coupling.error_rate !== undefined)
                couplingErrors.set(key, coupling.error_rate);
            if (coupling.duration_ns !== undefined)
                couplingDurations.set(key, coupling.duration_ns);
        }
    }
    const qubitErrors = new Map();
    if (profile) {
        for (const qubit of profile.qubits) {
            if (qubit.single_qubit_error !== undefined)
                qubitErrors.set(qubit.index, qubit.single_qubit_error);
        }
    }
        ...
        switch (operation.kind) {
            case "gate": {
                ...
                if (operation.qubits.length === 1) {
                    ...
                    if (error === undefined)
                        tally.errorsKnown = false;
                    else
                        tally.successProbability *= 1 - error;
                }
                    ...
                    if (duration === undefined)
                        tally.durationKnown = false;
                    else
                        tally.durationNs += duration;
                }
                if (T_GATES.has(name))
                    tally.t += 1;
                else if (CLIFFORD_GATES.has(name))
                    tally.clifford += 1;
    ...
    if (tally.unsupported > 0) {
        notes.push(`${tally.unsupported} gate(s) are neither Clifford nor T, so the T count understates the ` +
...
function collectGateNames(circuit) {
    const names = [];
    const visit = (operation) => {
        if (operation.kind === "gate")
            names.push(operation.name.toLowerCase());
 ...
 */
export function resourceEstimatesComparable(left, right) {
    const reasons = [];
    if (left.assumptions.estimator !== right.assumptions.estimator) {
        reasons.push(`Different estimators ('${left.assumptions.estimator}' and '${right.assumptions.estimator}') define ` +
            "these quantities differently.");
    }
    else if (left.assumptions.estimator_version !== right.assumptions.estimator_version) {
        reasons.push(`Same estimator at different versions ('${left.assumptions.estimator_version}' and ` +
