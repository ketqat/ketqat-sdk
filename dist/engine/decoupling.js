...
function touchedQubits(operation) {
 ...
 */
export function applyDynamicalDecoupling(circuit, options = {}) {
    ...
    const layered = [];
    const allQubits = new Set();
    const qubitByKey = new Map();
    for (const operation of circuit.operations) {
        ...
        for (const bit of bits)
            layerOf.set(key(bit), layer);
        layered.push({ operation, layer });
    }
    const busyInLayer = new Map();
    const stopsIdling = new Map();
    for (const { operation, layer } of layered) {
        if (layer < 0)
            continue;
        const set = busyInLayer.get(layer) ?? new Set();
    ...
    for (let layer = 1; layer <= maxLayer; layer += 1) {
        const busy = busyInLayer.get(layer) ?? new Set();
        if (busy.size === 0)
            continue;
        for (const qubitKey of allQubits) {
            if (busy.has(qubitKey))
                continue;
            const collapsesAt = stopsIdling.get(qubitKey);
            if (collapsesAt !== undefined && layer > collapsesAt)
                continue;
            // Idle only counts once the qubit has actually entered the circuit.
            const firstUse = layered.find((entry) => entry.layer >= 0 && touchedQubits(entry.operation).some((b) => key(b) === qubitKey));
            if (!firstUse || firstUse.layer > layer)
                continue;
        ...
        if (layer > current) {
            for (const bit of inserts.get(layer) ?? []) {
                for (const pulse of pulses) {
                    operations.push({ kind: "gate", name: pulse, qubits: [bit], parameters: [] });
                    emitted += 1;
                }
            }
            current = layer;
    ...
    const warnings = [];
    if (!canHelp) {
        warnings.push(`Dynamical decoupling cannot suppress this engine's noise model. Depolarizing noise is ` +
...
//# sourceMappingURL=decoupling.js.map