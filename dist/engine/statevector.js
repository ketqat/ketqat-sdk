...
export function zeroState(qubitCount) {
    if (qubitCount > MAX_SIMULATED_QUBITS) {
        throw new SimulationError(`Exact statevector simulation is limited to ${MAX_SIMULATED_QUBITS} qubits; got ${qubitCount}. ` +
...
function singleQubitMatrix(name, parameters) {
    ...
    switch (name) {
        ...
        default:
            throw new SimulationError(`Gate '${name}' is not supported by the statevector backend. ` +
 ...
 */
function u3Matrix(theta, phi, lambda) {
...
function requireParameters(name, parameters, count) {
...
function applySingleQubit(state, matrix, qubit) {
    ...
    for (let base = 0; base < real.length; base += stride << 1) {
        for (let offset = 0; offset < stride; offset += 1) {
            ...
            const ar = real[zero];
            const ai = imaginary[zero];
            const br = real[one];
            const bi = imaginary[one];
...
function applyControlled(state, matrix, controls, target) {
    const { real, imaginary } = state;
    ...
    for (let base = 0; base < real.length; base += stride << 1) {
        for (let offset = 0; offset < stride; offset += 1) {
            const zero = base + offset;
            if ((zero & controlMask) !== controlMask)
                continue;
            const one = zero + stride;
            const ar = real[zero];
            const ai = imaginary[zero];
...
function applySwap(state, a, b) {
    ...
    for (let index = 0; index < real.length; index += 1) {
        ...
        if (hasA && !hasB) {
            ...
            const tr = real[index];
            const ti = imaginary[index];
            real[index] = real[partner];
            imaginary[index] = imaginary[partner];
            real[partner] = tr;
...
export function probabilityOfOne(state, qubit) {
    const mask = 1 << qubit;
    let total = 0;
    for (let index = 0; index < state.real.length; index += 1) {
        if ((index & mask) !== 0) {
            const re = state.real[index];
            const im = state.imaginary[index];
            total += re * re + im * im;
        }
    }
    return total;
...
function collapse(state, qubit, outcome) {
    const mask = 1 << qubit;
    let norm = 0;
    for (let index = 0; index < state.real.length; index += 1) {
    ...
    for (let index = 0; index < state.real.length; index += 1) {
        state.real[index] = state.real[index] * scale;
        state.imaginary[index] = state.imaginary[index] * scale;
    }
}
function buildLayout(circuit) {
    const qubitOffsets = new Map();
    let qubitTotal = 0;
    for (const register of circuit.qubit_registers) {
        qubitOffsets.set(register.name, qubitTotal);
        qubitTotal += register.size;
    }
    const clbitOffsets = new Map();
    let clbitTotal = 0;
    ...
    return {
        index: (bit) => {
            ...
            if (offset === undefined)
                throw new SimulationError(`Unknown qubit register '${bit.register}'.`);
            return offset + bit.index;
        },
        clbitIndex: (bit) => {
            const offset = clbitOffsets.get(bit.register);
            if (offset === undefined)
                throw new SimulationError(`Unknown classical register '${bit.register}'.`);
            return offset + bit.index;
        },
...
function evaluateParameters(operation) {
        ...
        switch (operation.kind) {
            case "gate": {
                ...
                if (TWO_QUBIT_GATES.has(name)) {
                    ...
                    const baseName = name === "cx" || name === "cnot"
                        ? "x"
                        : name === "cz"
                            ? "z"
                            : name === "cy"
                ...
                if (qubits.length !== 1) {
                    throw new SimulationError(`Gate '${operation.name}' with ${qubits.length} qubits is not supported by the statevector backend.`);
                }
                applySingleQubit(state, singleQubitMatrix(name, parameters), qubits[0]);
                return;
            }
            case "measure": {
            ...
            case "conditional": {
                ...
                const observed = operation.bit === undefined
                    ? registerValue(operation.register)
                    : clbits[layout.clbitIndex({ register: operation.register, index: operation.bit })];
...
function applyControlledSwap(state, control, a, b) {
    const { real, imaginary } = state;
...
export function totalQubitCount(circuit) {
 ...
 */
export function simulateStatevector(circuit, options = {}) {
    ...
    if (layout.clbitCount === 0) {
        throw new SimulationError("This circuit declares no classical bits, so there is nothing to sample. Add a measurement, " +
