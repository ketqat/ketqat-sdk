import { MAX_SIMULATED_QUBITS, simulateStatevector, totalQubitCount, zeroState, } from "./statevector.js";
function stateFromCircuit(circuit) {
 ...
 * every measurement.
 */
export function compareStates(left, right, ignoreGlobalPhase = true) {
    ...
    for (let index = 0; index < left.real.length; index += 1) {
        const ar = left.real[index];
        const ai = left.imaginary[index];
        const br = right.real[index];
        const bi = right.imaginary[index];
    ...
    for (let index = 0; index < left.real.length; index += 1) {
        ...
        const difference = Math.hypot(left.real[index] - rotatedReal, left.imaginary[index] - rotatedImaginary);
 ...
 */
export function checkCircuitEquivalence(left, right, options = {}) {
    ...
    if (leftQubits > maxQubits) {
        return {
            ...
            reason: `Exact comparison needs 2^${leftQubits} amplitudes, above the ${maxQubits}-qubit limit. ` +
    ...
    try {
        ...
        return {
            ...
            method: "statevector",
            qubit_count: leftQubits,
            reason: `Could not simulate both circuits: ${error.message}`,
        };
    }
    ...
    return {
        ...
        global_phase_ignored: ignoreGlobalPhase,
        qubit_count: leftQubits,
        counterexample: `Maximum amplitude difference ${comparison.max_amplitude_difference.toExponential(3)} exceeds ` +
 ...
 */
export function compareShotResults(left, right, options = {}) {
