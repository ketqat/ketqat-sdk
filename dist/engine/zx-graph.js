...
export function isInterior(graph, id) {
 ...
 * rather than assumed.
 */
export function graphToMatrix(graph) {
    const ids = graph.spiders.map((spider) => spider.id);
    const position = new Map(ids.map((id, index) => [id, index]));
    const spiderCount = ids.length;
    if (spiderCount > 14) {
        throw new ZxGraphError(`${spiderCount} spiders means 2^${spiderCount} terms; this evaluator refuses rather than crawling.`);
    }
    const inputDimension = 1 << graph.inputs.length;
    const outputDimension = 1 << graph.outputs.length;
    const real = Array.from({ length: outputDimension }, () => new Array(inputDimension).fill(0));
    const imaginary = Array.from({ length: outputDimension }, () => new Array(inputDimension).fill(0));
    ...
    for (let inputBits = 0; inputBits < inputDimension; inputBits += 1) {
        for (let outputBits = 0; outputBits < outputDimension; outputBits += 1) {
            ...
            for (let assignment = 0; assignment < 1 << spiderCount; assignment += 1) {
                const value = (id) => (assignment >> position.get(id)) & 1;
                ...
                for (const spider of graph.spiders) {
                    if (value(spider.id) === 1)
                        angle += Math.PI * spider.phase;
                }
                for (const [a, b] of graph.edges) {
            ...
            ;
            real[outputBits][inputBits] = sumReal;
            imaginary[outputBits][inputBits] = sumImaginary;
        }
    }
 ...
 */
export function sameUpToScalar(left, right, tolerance = 1e-9) {
    const rows = left.real.length;
    const columns = left.real[0]?.length ?? 0;
    if (right.real.length !== rows || (right.real[0]?.length ?? 0) !== columns) {
    ...
    for (let row = 0; row < rows; row += 1) {
        for (let column = 0; column < columns; column += 1) {
            const magnitude = Math.hypot(left.real[row][column], left.imaginary[row][column]);
    ...
    if (bestMagnitude < tolerance) {
        ...
        for (let row = 0; row < rows; row += 1) {
            for (let column = 0; column < columns; column += 1) {
                maxRight = Math.max(maxRight, Math.hypot(right.real[row][column], right.imaginary[row][column]));
            }
        }
        return { equal: maxRight < tolerance, scalar: null, maxDifference: maxRight };
    }
    const lr = left.real[bestRow][bestColumn];
    const li = left.imaginary[bestRow][bestColumn];
    const rr = right.real[bestRow][bestColumn];
    const ri = right.imaginary[bestRow][bestColumn];
    ...
    for (let row = 0; row < rows; row += 1) {
        for (let column = 0; column < columns; column += 1) {
            const scaledReal = scalarReal * right.real[row][column] -
                scalarImaginary * right.imaginary[row][column];
            const scaledImaginary = scalarReal * right.imaginary[row][column] +
                scalarImaginary * right.real[row][column];
            maxDifference = Math.max(maxDifference, Math.hypot(left.real[row][column] - scaledReal, left.imaginary[row][column] - scaledImaginary));
        }
    }
    return {
...
function complementAmong(graph, vertices) {
 ...
 */
export function localComplementation(input, id) {
    const graph = {
    ...
    for (const neighbour of affected) {
        ...
        if (target)
            target.phase = normalisePhase(target.phase + shift);
    }
 ...
 */
export function pivot(input, u, v) {
    const graph = {
        spiders: input.spiders.map((spider) => ({ ...spider })),
        edges: input.edges.map(([a, b]) => [a, b]),
        inputs: [...input.inputs],
    ...
    const shiftPhases = (ids, amount) => {
        if (amount === 0)
            return;
        for (const id of ids) {
            const target = graph.spiders.find((candidate) => candidate.id === id);
            if (target)
                target.phase = normalisePhase(target.phase + amount);
        }
    };
    shiftPhases(onlyU, b);
    shiftPhases(onlyV, a);
 ...
 */
export function applyVerified(graph, rewrite) {
    ...
    try {
        ...
        return {
            ...
            maxDifference: null,
            detail: `Could not verify: ${error.message} The rewrite is not reported as an optimisation without a check.`,
        };
    }
