...
import { MCP_TOOLS, callTool } from "./index.js";
...
export function handleRequest(request) {
    ...
    switch (request.method) {
        case "initialize": {
            ...
            const agreed = requested && SUPPORTED_PROTOCOL_VERSIONS.includes(requested)
                ? requested
        ...
        case "tools/call": {
            ...
            try {
                ...
                return {
                    jsonrpc: "2.0",
                    id,
                    error: { code: INTERNAL_ERROR, message: `${params.name} threw: ${error.message}` },
                };
            }
...
export function serveStdio(options = {}) {
    ...
    if (mutating.length > 0) {
        return Promise.reject(new Error(`Refusing to serve: ${mutating.map((tool) => tool.name).join(", ")} ${mutating.length === 1 ? "is" : "are"} not read-only. stdio has no authentication, so this transport serves read-only tools only.`));
    }
        ...
        input.on("data", (chunk) => {
            ...
            while (newline !== -1) {
                ...
                try {
                    ...
                    log(`malformed frame: ${error.message}`);
