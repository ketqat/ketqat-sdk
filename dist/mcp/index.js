...
const LossReportOutput = z.array(z.object({
    feature: z.string(),
...
function circuitOf(qasm) {
    return parseQasm3(qasm);
}
export const inspectCircuitTool = {
    name: "inspect_circuit",
    title: "Inspect a quantum circuit",
    description: "Parse OpenQASM 3 and report structure: qubit and clbit counts, gate counts, depth, and whether " +
...
export const convertCircuitTool = {
    name: "convert_circuit",
    title: "Convert a circuit to canonical OpenQASM 3",
    description: "Re-emit a circuit as canonical OpenQASM 3. Any construct the adapter cannot represent is reported " +
...
const SimulateInput = z.object({
    qasm: z.string().min(1),
    shots: z.number().int().positive().max(200000).optional(),
    seed: z.number().int().nonnegative().optional().describe("Omit for a non-reproducible run."),
    noise: NoiseModelSchema.optional(),
});
export const simulateCircuitTool = {
    name: "simulate_circuit",
    title: "Simulate a quantum circuit",
    description: "Exact statevector simulation. With shots, returns measurement counts; without shots and without " +
...
export const estimateResourcesTool = {
    name: "estimate_resources",
    title: "Estimate circuit resources",
    description: "Static resource estimate with its assumptions attached: estimator, version, gate set, and what the " +
...
export const transpileForHardwareTool = {
    name: "transpile_for_hardware",
    title: "Transpile a circuit onto a hardware snapshot",
    description: "Route a circuit onto a device's coupling graph, inserting SWAPs. Reports SWAP count and depth " +
...
export const optimizeWithZxTool = {
    name: "optimize_with_zx",
    title: "Optimize a circuit with ZX rewrites",
    description: "Apply a bounded set of ZX rewrites and return the result together with checked equivalence " +
...
export const checkEquivalenceTool = {
    name: "check_circuit_equivalence",
    title: "Check whether two circuits are equivalent",
    description: "Compare two circuits by exact simulation. Returns FAILED only with a counterexample, and " +
