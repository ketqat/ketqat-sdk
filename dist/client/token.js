...
export const ACCEPTED_TOKEN_VARIABLES = [CANONICAL_TOKEN_VARIABLE, "KETQAT_TOKEN"];
...
export class AmbiguousApiTokenError extends Error {
    constructor(names) {
        super(`${names.join(" and ")} are both set, to different values. ` +
...
export function missingApiTokenMessage() {
    return (`No API token. Set ${CANONICAL_TOKEN_VARIABLE} in the environment ` +
 ...
 */
export function resolveApiToken(environment = process.env) {
    const present = [];
