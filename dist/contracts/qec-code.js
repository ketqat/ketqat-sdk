...
export const QEC_CODE_CATALOG = [
    {
        slug: "rotated-surface-code-memory-x",
        name: "Rotated surface code (X memory)",
        families: ["STABILIZER", "CSS", "TOPOLOGICAL", "SURFACE"],
        description: "Planar topological code on a rotated lattice, preserving one logical qubit through repeated " +
    ...
    },
    {
        ...
        families: ["STABILIZER", "CSS", "TOPOLOGICAL", "SURFACE"],
        description: "Unrotated surface code memory; uses more physical qubits per distance than the rotated layout.",
        requires_mid_circuit_measurement: true,
        requires_feed_forward: false,
        requires_nonlocal_connectivity: false,
        requires_loss_detection: false,
    ...
    },
    {
        ...
        description: "One-dimensional classical repetition code protecting against a single error type. Useful as a " +
...
export function getQecCode(slug) {
    return QEC_CODE_CATALOG.find((code) => code.slug === slug);
}
export function qecCodesInFamily(family) {
    const upper = family.toUpperCase();
    return QEC_CODE_CATALOG.filter((code) => code.families.includes(upper));
}
/**
 ...
 */
export function assessQecSuitability(code, capabilities) {
    const blockers = [];
    let level = "THEORETICALLY_SUITABLE";
    ...
    if (code.requires_nonlocal_connectivity &&
    ...
    return {
        ...
        level,
        blockers,
        evidence: "Derived from the capability fields of the supplied hardware snapshot. This is a capability " +
            "match, not an experimental result: only a recorded run may raise the level to SIMULATED or " +
            "DEMONSTRATED.",
    };
}
//# sourceMappingURL=qec-code.js.map