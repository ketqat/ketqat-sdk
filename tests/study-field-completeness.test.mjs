...
const SCHEMA_FOR_KIND = {
  ...
  confirmation_receipt: study.ConfirmationReceiptSchema,
  study_task_authorization: study.StudyTaskAuthorizationSchema,
  task_outcome: study.TaskOutcomeSchema,
  evidence_node: study.EvidenceNodeSchema,
  evidence_edge: study.EvidenceEdgeSchema,
  review_record: study.ReviewRecordSchema,
  reproduction_record: study.ReproductionRecordSchema,
...
test("every record kind in the registry has a schema to be checked against", () => {
  ...
  // A floor rather than an exact count. The comparison above already ties the
  // two lists to each other, so the only failure a number adds is the one where
  // *both* went empty and the file passed by walking nothing -- and a floor
  // catches that without having to be edited every time a record kind is added,
  // which is an edit whose only effect is to make the assertion true again.
  assert.ok(kinds.length >= 11, `the registry declares ${kinds.length} record kinds`)
})
...
test("the check is not vacuous: an unclassified schema field fails it", () => {
  ...
  const entry = STUDY_RECORD_KINDS.find((candidate) => candidate.record_kind === "study_task_authorization")
  const clean = disagreements(entry.shape, SCHEMA_FOR_KIND.study_task_authorization)
  assert.deepEqual(clean.unclassified, [])

  const withSmuggledField = SCHEMA_FOR_KIND.study_task_authorization.extend({
...
test("the check is not vacuous: a nested unclassified field fails it too", () => {
  ...
  const entry = STUDY_RECORD_KINDS.find((candidate) => candidate.record_kind === "study_task_authorization")
  const withNestedField = SCHEMA_FOR_KIND.study_task_authorization.extend({
...
test("the check is not vacuous: a field on one union variant fails it too", () => {
  // The union's own version of the silent omission. `study_event` is a
  // discriminated union of twenty-two variants over one record kind, so a
  // payload field added to the twenty-second is exactly as invisible to a digest
  // as one added at the root -- and a walk that looked only at the first option
  // would report nothing and pass. The field is added to a variant chosen from
  // the end of the union for that reason.
  const entry = STUDY_RECORD_KINDS.find((candidate) => candidate.record_kind === "study_event")
  const clean = disagreements(entry.shape, SCHEMA_FOR_KIND.study_event)
  assert.deepEqual(clean.unclassified, [])

  const options = SCHEMA_FOR_KIND.study_event._def.schema._def.options
  assert.equal(options.length, 22, "the walk has to reach every variant, so count them")
  const last = options[options.length - 1]
  const withSmuggledPayload = z.discriminatedUnion("event_type", [
    ...options.slice(0, -1),
    last.extend({ smuggled_payload_field: z.string() }),
  ])
  const broken = disagreements(entry.shape, withSmuggledPayload)
  assert.deepEqual(broken.unclassified, ["smuggled_payload_field"])
  assert.deepEqual(broken.undeclared, [])
})
...
test("the check is not vacuous: a classification with no schema field fails it", () => {
  ...
  const broken = disagreements(stale, SCHEMA_FOR_KIND.study_task_authorization)
...
test("the classification walk reaches nested and array-of-object shapes", () => {
  ...
  const classified = flattenShapeClasses(entry.shape)
  assert.equal(classified.get("nodes.claim.value_ref.node_hash"), "SEMANTIC")
  assert.equal(classified.get("nodes.claim.subject_ref.record_slug"), "RECORD_ONLY")
  assert.equal(classified.get("nodes.quantity.uncertainty.basis"), "SEMANTIC")
  assert.equal(classified.get("environment.packages.name"), "SEMANTIC")
  // A figure is a spec now rather than markup, so the deep path runs through
  // the coordinates: a point names a node, and that reference is semantic even
  // though the figure it is drawn in is presentation.
  assert.equal(classified.get("figures.spec.series.points.y.node_hash"), "SEMANTIC")
  assert.equal(classified.get("figures.title"), "RECORD_ONLY")
  assert.equal(classified.get("tables.rows.cells.node_hash"), "SEMANTIC")
  // The outer field decides whether the CSV artifact is read at all; its own
  // fields are facts about bytes and are semantic, which is only visible once
  // an outer field selects them.
  assert.equal(classified.get("tables.csv_artifact"), "RECORD_ONLY")
  assert.equal(classified.get("tables.csv_artifact.content_hash"), "SEMANTIC")
  assert.equal(classified.get("report.sections.segments.text"), "RECORD_ONLY")
  assert.equal(classified.get("recipe.resource_limits.max_memory_bytes"), "SEMANTIC")
  assert.equal(classified.get("check_ledger.tool.version"), "RECEIPT_ONLY")
...
test("the shape tables Python projects from are the ones this registry declares", () => {
  ...
  // The three tables Python reads besides the shapes. The ceilings and the
  // graph's own rules are decisions this repository makes once, and a second
  // hand-written copy in Python would be a second thing that can drift with
  // nothing watching it -- so they are emitted, and compared here against the
  // source the same way.
  assert.deepEqual(emitted.package_limits, { ...STUDY_PACKAGE_LIMITS })
  assert.deepEqual(
    emitted.evidence_ground_rules,
    EVIDENCE_GROUND_RULES.map((rule) => ({ node_kind: rule.node_kind, grounds: rule.grounds })),
  )
  assert.deepEqual(
    emitted.evidence_edge_matrix,
    EVIDENCE_EDGE_MATRIX.map((rule) => ({
      from_kind: rule.from_kind,
      edge_kind: rule.edge_kind,
      to_kind: rule.to_kind,
    })),
  )
  assert.ok(emitted.evidence_edge_matrix.length >= 40, "the matrix is not a matrix")
})
