...
export function evaluateParameter(value: number | string): number {
...
class ExpressionParser {
  ...
  private parseAtom(): number {
    ...
    const numberMatch = /^\d+\.?\d*(?:[eE][+-]?\d+)?|^\.\d+(?:[eE][+-]?\d+)?/.exec(
    ...
    if (identifierMatch) {
      ...
      if (constant === undefined) {
        ...
        throw new ParameterError(
          `Parameter '${name}' has no value. Bind free parameters before simulating.`,
        )
      }
      return constant
    }
  ...
  private skipWhitespace(): void {
    while (this.position < this.source.length && /\s/.test(this.source[this.position] as string)) {
      this.position += 1
    }
  }
  ...
  expectEnd(): void {
    ...
    if (this.position < this.source.length) {
      throw new ParameterError(
        `Unexpected '${this.source.slice(this.position)}' in parameter expression '${this.source}'.`,
      )
    }
  }
}
