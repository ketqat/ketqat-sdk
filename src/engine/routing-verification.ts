...
import { couplingAdjacency, type HardwareProfile } from "../hardware/profile.js"
...
export interface RoutingVerification {
  ...
  offCouplingGates: Array<{ name: string; qubits: number[] }>
 ...
 */
export function unpermuteAmplitudes(
  ...
  if (amplitudes.real.length !== size) {
    throw new RoutingVerificationError(
      `Expected ${size} amplitudes for ${totalQubits} qubits, got ${amplitudes.real.length}.`,
    )
  }
  ...
  for (let routedIndex = 0; routedIndex < size; routedIndex += 1) {
    ...
    for (const [logical, physical] of finalLayout.entries()) {
      if ((routedIndex >> physical) & 1) logicalIndex |= 1 << logical
    }
    real[logicalIndex] = amplitudes.real[routedIndex] as number
    imaginary[logicalIndex] = amplitudes.imaginary[routedIndex] as number
  }
  return { real, imaginary }
}

function widthOf(circuit: QuantumCircuit): number {
  return circuit.qubit_registers.reduce((total, register) => total + register.size, 0)
}

function stripMeasurements(circuit: QuantumCircuit): QuantumCircuit {
  return {
    ...circuit,
...
export function verifyRouting(
  ...
  const offCouplingGates: Array<{ name: string; qubits: number[] }> = []
  for (const operation of routed.operations) {
    if (operation.kind !== "gate" || operation.qubits.length !== 2) continue
    const first = operation.qubits[0]?.index as number
    const second = operation.qubits[1]?.index as number
  ...
  if (width > MAX_VERIFIABLE_QUBITS) {
    return {
      ...
      swapCount,
      detail:
        `${width} qubits is beyond the ${MAX_VERIFIABLE_QUBITS}-qubit limit for exact comparison, so ` +
  ...
  try {
    ...
    return {
      ...
      maxAmplitudeDifference: null,
      swapCount,
      detail: `Could not simulate for comparison: ${(error as Error).message}`,
    }
  }
  ...
  for (let index = 0; index < originalState.real.length; index += 1) {
    paddedOriginal.real[index] = originalState.real[index] as number
    paddedOriginal.imaginary[index] = originalState.imaginary[index] as number
  }
  ...
  for (let index = 0; index < routedState.real.length; index += 1) {
    paddedRouted.real[index] = routedState.real[index] as number
    paddedRouted.imaginary[index] = routedState.imaginary[index] as number
  }
  ...
  for (let physical = 0; physical < width; physical += 1) {
    if (!used.has(physical)) layout.push(physical)
  }
  ...
  for (let index = 0; index < size; index += 1) {
    maxDifference = Math.max(
      maxDifference,
      Math.hypot(
        (paddedOriginal.real[index] as number) - (unpermuted.real[index] as number),
        (paddedOriginal.imaginary[index] as number) - (unpermuted.imaginary[index] as number),
      ),
    )
  }
