...
export function isCliffordGate(name: string, parameters: ReadonlyArray<number | string>): boolean {
  const lower = name.toLowerCase()
  if (CLIFFORD_NAMES.has(lower)) return true
  if (lower === "t" || lower === "tdg") return false
 ...
 */
export function nearCliffordVariant(
  circuit: QuantumCircuit,
  random: () => number,
  substitutionRate = 0.5,
): { circuit: QuantumCircuit; substitutions: number } {
  let substitutions = 0
  const operations: Operation[] = circuit.operations.map((operation) => {
    if (operation.kind !== "gate") return operation
    if (isCliffordGate(operation.name, operation.parameters)) return operation
    if (random() > substitutionRate) return operation

    substitutions += 1
    const lower = operation.name.toLowerCase()
    if (lower === "t") return { ...operation, name: "s", parameters: [] }
    if (lower === "tdg") return { ...operation, name: "sdg", parameters: [] }
 ...
 */
export function exactExpectation(circuit: QuantumCircuit, clbitIndex = 0): number {
  ...
  if (target === null) {
    throw new CdrError(
      `No measurement writes classical bit ${clbitIndex}, so there is no observable to compute.`,
    )
  }
  ...
  const unmeasured: QuantumCircuit = {
    ...circuit,
    operations: circuit.operations.filter(
      (operation) => operation.kind !== "measure" && operation.kind !== "reset",
    ),
  }
...
export function cliffordDataRegression(
  circuit: QuantumCircuit,
  noise: NoiseModel,
  options: CdrOptions = {},
): CdrResult {
  ...
  if (trainingCircuits < 3) {
    throw new CdrError(
      `A regression through ${trainingCircuits} points cannot be assessed. At least 3 are needed for ` +
        "R^2 to mean anything.",
    )
  }

  const nonClifford = circuit.operations.filter(
    (operation) => operation.kind === "gate" && !isCliffordGate(operation.name, operation.parameters),
  ).length

  const warnings: string[] = []
  if (nonClifford === 0) {
    warnings.push(
      "This circuit is already Clifford, so every training variant is identical to it and the " +
  ...
  if (variance < 1e-12) {
    throw new CdrError(
      "The training circuits all produced essentially the same noisy value, so the slope is " +
  ...
  if (!interpolating) {
    warnings.push(
      `The target's noisy value (${raw.toFixed(4)}) lies outside the training range ` +
  ...
  if (targetIsConstant) {
    warnings.push(
      "Every training circuit had the same exact value, so the regression is vacuous: it predicts that " +
  ...
  if (!targetIsConstant && rSquared < 0.9) {
    warnings.push(
      `R^2 is ${rSquared.toFixed(3)}, so a straight line describes this noise map poorly. The mitigated ` +
  ...
  if (mitigated < -1 || mitigated > 1) {
    warnings.push(
      `The mitigated value ${mitigated.toFixed(4)} is outside [-1, 1] and so unphysical. It is reported ` +
