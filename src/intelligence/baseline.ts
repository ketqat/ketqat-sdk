  ...
  .superRefine((baseline, context) => {
    if (baseline.evidence === "UNKNOWN" && (baseline.runtime !== null || baseline.monetary_cost !== null)) {
      context.addIssue({
        ...
        message:
          "A baseline classified UNKNOWN cannot carry a runtime or a cost. If a number is known, say how it was obtained.",
        path: ["evidence"],
      })
    }
    if (baseline.evidence === "MEASURED" && baseline.measured_on === null) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "A MEASURED baseline must record when it was measured. Classical hardware improves; an undated measurement " +
    ...
    if (baseline.runtime === null && baseline.monetary_cost === null) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "A baseline with neither a runtime nor a cost supports no comparison. Omit the baseline entirely rather than " +
...
export function supportsRuntimeComparison(baseline: ClassicalBaseline | null): boolean {
...
export function supportsCostComparison(baseline: ClassicalBaseline | null): boolean {
