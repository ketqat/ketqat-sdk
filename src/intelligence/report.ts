import { z } from "zod"
import { INTELLIGENCE_SCHEMA_VERSION, isKnown, type Quantity } from "./measurement.js"
 ...
 */
export function renderQuantity(value: Quantity): string {
  if (!isKnown(value)) return `unknown (${value.limitations[0] ?? "not computed"})`
  const magnitude =
    Math.abs(value.value) >= 1e6 || (Math.abs(value.value) < 1e-3 && value.value !== 0)
      ? value.value.toExponential(3)
      : value.value.toLocaleString("en-US", { maximumFractionDigits: 4 })
  const prefix =
    value.bound === "UPPER_BOUND" ? "at most " : value.bound === "LOWER_BOUND" ? "at least " : ""
  return `${prefix}${magnitude} ${value.unit}`
}

export function buildReport(bundle: ResourceIntelligenceBundle): DecisionReport {
  const summary: string[] = []
  if (bundle.is_demo) {
    summary.push(
      "This report is built from a demonstration fixture. It is not evidence about any real workload, device, " +
        "organisation, or quantum advantage claim.",
    )
  }
  summary.push(
    `Workload: ${bundle.workload.name}. ${bundle.workload.logical.logical_qubits} logical qubits, ` +
  ...
  summary.push(
    bundle.classical_baseline === null
  ...
  for (let index = 0; index < bundle.scenarios.length; index += 1) {
    ...
    summary.push(
      `${scenario.name}: ${assessment.status}. ` +
        (estimate.feasible
          ...
          : (estimate.infeasibility_reason as string)),
    )
  }
    ...
    statements:
      bundle.classical_baseline === null
        ...
        : [
            ...
            `Cost: ${
              bundle.classical_baseline.monetary_cost
  ...
  sections.push({
    ...
    statements: bundle.assessments.flatMap((assessment) => [
      ...
      ...assessment.dimensions.map(
        (dimension) => `  ${dimension.dimension}: ${dimension.status} -- ${dimension.explanation}`,
      ),
  ...
  sections.push({
    ...
    statements: [
      ...
      ...bundle.comparison.rows.map(
        (row) =>
          `${row.scenario_name}: ${row.decision_status}, ` +
...
export function reportToCsv(bundle: ResourceIntelligenceBundle): string {
