import { z } from "zod"
import { INTELLIGENCE_SCHEMA_VERSION, isKnown, type Contract } from "./measurement.js"
import { QuantumWorkloadSchema, type QuantumWorkload } from "./workload.js"
...
import { ResourceEstimateSnapshotSchema, estimateForScenario, type ResourceEstimateSnapshot } from "./estimate.js"
import { AdvantageThresholdSchema, computeAdvantageThresholds, type AdvantageThreshold } from "./thresholds.js"
import { DecisionAssessmentSchema, assessDecision, type DecisionAssessment } from "./decision.js"
import {
...
export interface ResourceIntelligenceBundle {
  ...
  is_demo: boolean
  generator: { name: string; version: string; schema_version: string }
...
export const ResourceIntelligenceBundleSchema: Contract<ResourceIntelligenceBundle> = z.object({
...
const AGGREGATION_POLICY =
  "No aggregate row is produced. Scenarios differ in their assumptions, and a mean of results computed under " +
 ...
 * canonicalization.
 */
export function buildBundle(input: BundleInput): ResourceIntelligenceBundle {
  ...
  const estimates: ResourceEstimateSnapshot[] = []
  const thresholds: AdvantageThreshold[] = []
  ...
  const comparison: ScenarioComparison = {
    ...
    rows: scenarios.map((scenario, index) =>
      comparisonRow(scenario, estimates[index], thresholds[index], assessments[index]),
    ),
  ...
  if (workload.is_demo) {
    limitations.push(
      "This bundle is built from a demonstration fixture. It is not evidence about any real workload, device, " +
  ...
  const withoutHash = {
    ...
    bundle_kind: "RESOURCE_INTELLIGENCE" as const,
 ...
 */
export function verifyBundle(candidate: unknown): Verification {
  ...
  const unwrapped =
    candidate && typeof candidate === "object" && "bundle" in (candidate as Record<string, unknown>)
      ? (candidate as { bundle: unknown }).bundle
      : candidate
  ...
  if (!hashMatches) {
    problems.push(
      `Reproducibility hash mismatch: the bundle claims ${bundle.reproducibility_hash} and its own contents ` +
        `canonicalize to ${rebuilt.reproducibility_hash}.`,
    )
  }

  const estimatesMatch =
    JSON.stringify(rebuilt.estimates) === JSON.stringify(bundle.estimates) &&
    JSON.stringify(rebuilt.thresholds) === JSON.stringify(bundle.thresholds)
  if (!estimatesMatch) {
    problems.push(
      "The stored estimates or thresholds do not match what the stored inputs and assumptions produce. The numbers " +
  ...
  if (!decisionMatches) {
    problems.push(
      "The stored decision assessments do not match what the stored estimates produce. A conclusion in this bundle " +
