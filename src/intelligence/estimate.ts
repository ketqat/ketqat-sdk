...
import {
  INTELLIGENCE_ESTIMATOR,
  INTELLIGENCE_ESTIMATOR_VERSION,
  type LayoutModel,
...
import {
  ...
  type FaultTolerantAssumptions,
} from "../engine/fault-tolerant.js"
...
export function occupiedLogicalQubits(algorithmQubits: number, layout: LayoutModel): number {
  if (layout === "BARE_REGISTER") return algorithmQubits
...
export function evaluate(workload: QuantumWorkload, scenario: ResourceScenario): CoreEvaluation {
  ...
  const assumptions: FaultTolerantAssumptions = {
  ...
  if (scenario.hardware.physical_error_rate >= scenario.qec.threshold) {
    return {
      ...
      infeasibilityReason:
        `A physical error rate of ${scenario.hardware.physical_error_rate} is at or above the ` +
  ...
  if (distance === null) {
    return {
      ...
      infeasibilityReason:
        "No code distance within the search bound meets this error budget. The budget is too tight for this " +
  ...
  const perPatch = (d: number): number => scenario.qec.qubits_per_logical_d_squared * d * d
  ...
  let factoryPhysical: number | null = 0
  let rawInputStates: number | null = 0
  let levels = 0
  ...
  const factoryShare =
    factoryPhysical === null || totalPhysical === null || totalPhysical === 0
      ? null
  ...
  let runtimeFactoryLimited: number | null = null
  ...
  const runtime =
    runtimeFactoryLimited === null ? null : Math.max(runtimeCycleLimited, runtimeFactoryLimited)
  const runtimeLimiter: RuntimeLimiter =
    runtimeFactoryLimited === null
      ? "NOT_DETERMINED"
  ...
  const achievedLogicalError =
    logicalErrorPerCycle(distance, assumptions) * logical.logical_qubits * logicalCycles
...
function countQuantity(value: number, unit: string, workload: QuantumWorkload): Quantity {
  return quantity({
    value,
    unit,
...
function sensitivityFor(
  ...
  base: CoreEvaluation,
): EstimateSensitivityPoint[] {
  const points: EstimateSensitivityPoint[] = []
  const relative = (value: number | null): number | null =>
    value === null || base.totalPhysical === null || base.totalPhysical === 0
      ? null
  ...
  for (const layout of ["BARE_REGISTER", "LATTICE_SURGERY_2D"] as const) {
  ...
  for (const factor of [0.1, 1, 10]) {
    ...
    if (raw <= scenario.factory.target_state_error) continue
...
export function estimateForScenario(
  ...
  const derived = (
    ...
    unit: string,
    source: string,
    options: { limitations?: string[]; uncertaintyFrom?: SensitivityParameter } = {},
  ): Quantity => {
    if (value === null) {
    ...
    if (options.uncertaintyFrom) {
      const scoped = sensitivity.filter(
        (point) => point.parameter === options.uncertaintyFrom && point.total_physical_qubits !== null,
      )
      const values = scoped.map((point) => point.total_physical_qubits as number)
      if (values.length > 0) {
        uncertainty = {
          kind:
            options.uncertaintyFrom === "LOGICAL_ERROR_PREFACTOR"
              ? ("MODEL_SPREAD" as const)
              : ("SENSITIVITY_RANGE" as const),
          low: Math.min(...values),
  ...
  const notComputable = (reason: string) => reason
  ...
  if (core.feasible && !core.distillationReachedTarget && core.magicStates > 0) {
    ...
    warnings.push(
      "The factory footprint could not be computed, so no total machine size is reported. The algorithm figure " +
        "below is a component of the requirement, not the requirement.",
    )
  }
  if (core.factoryShare !== null && core.factoryShare > 0.5) {
    warnings.push(
      `The magic-state factory is ${(core.factoryShare * 100).toFixed(0)}% of the machine -- larger than the ` +
  ...
  if (scenario.hardware.basis === "ROADMAP") {
    warnings.push(
      "The hardware parameters come from a published roadmap, not an observation. Every figure below is " +
        "conditional on that roadmap being met.",
    )
  }
  if (workload.is_demo) {
    warnings.push(
      "This is a demonstration fixture. It is not evidence about any real workload, device, or organisation.",
    )
  }
  ...
  if (core.codeDistance !== null) {
    exactArithmetic.push(
      `Per-patch footprint: ${scenario.qec.qubits_per_logical_d_squared} x ${core.codeDistance}^2 = ` +
        `${scenario.qec.qubits_per_logical_d_squared * core.codeDistance * core.codeDistance} physical qubits.`,
    )
    exactArithmetic.push(
      `Cycle-limited runtime: ${core.logicalCycles} cycles x ${core.codeDistance} rounds x ` +
  ...
  if (core.magicStates > 0 && core.distillationReachedTarget) {
    exactArithmetic.push(
      `Distillation: ${core.distillationLevels} level(s) of 15-to-1, ${15 ** core.distillationLevels} raw states ` +
  ...
  return ResourceEstimateSnapshotSchema.parse({
    ...
    code_distance: derived(
      core.codeDistance,
      "code distance",
      core.infeasibilityReason ?? "Smallest odd distance whose logical error rate meets the per-qubit-per-cycle budget.",
    ),
    ...
    raw_magic_state_input_count: derived(
      core.rawInputStates,
      "raw states",
      core.distillationReachedTarget
        ? `Magic states x 15^${core.distillationLevels}, the 15-to-1 input multiplier.`
        : notComputable(`Not computed: ${core.distillationReason}`),
    ),
    distillation_levels: derived(
      core.magicStates > 0 && !core.distillationReachedTarget ? null : core.distillationLevels,
      "levels",
      core.distillationReason,
    ),
    algorithm_physical_qubits: derived(
      core.algorithmPhysical,
      "physical qubits",
      core.infeasibilityReason ?? "The algorithm register's own patches. Excludes routing space and the factory.",
      {
        limitations: [
    ...
    layout_adjusted_physical_qubits: derived(
      core.layoutAdjustedPhysical,
      "physical qubits",
      core.infeasibilityReason ?? `Algorithm patches plus routing space under ${scenario.layout_model}.`,
      {
        limitations: ["Excludes the magic-state factory."],
        uncertaintyFrom: "LAYOUT_MODEL",
      },
    ),
    factory_physical_qubits: derived(
      core.factoryPhysical,
      "physical qubits",
    ...
    total_physical_qubits: derived(
      core.totalPhysical,
      "physical qubits",
      core.totalPhysical === null
        ? notComputable(
            core.feasible
              ? `Not computed: the factory could not be sized. ${core.distillationReason}`
              : (core.infeasibilityReason as string),
          )
        : "Layout-adjusted algorithm footprint plus factory footprint.",
      { uncertaintyFrom: "PHYSICAL_ERROR_RATE" },
    ),
    factory_share: derived(
      core.factoryShare,
      "fraction",
      core.factoryShare === null
        ? notComputable("Not computed: the factory could not be sized.")
        : "Factory physical qubits divided by total physical qubits.",
    ),
    occupied_logical_qubits: derived(
      core.occupiedLogical,
      "logical qubits",
    ...
    runtime: derived(
      ...
      core.runtime === null
        ...
        : core.runtimeLimiter === "MAGIC_STATE_THROUGHPUT"
          ...
          : "Limited by logical cycles.",
      { uncertaintyFrom: "CYCLE_TIME" },
    ),
    runtime_cycle_limited: derived(
      core.runtimeCycleLimited,
      "s",
      core.runtimeCycleLimited === null
        ? notComputable(core.infeasibilityReason as string)
        : "Logical cycles x code distance x surface-code cycle time.",
    ),
    runtime_factory_limited: derived(
      core.runtimeFactoryLimited,
      "s",
    ...
    magic_state_throughput: derived(
      core.throughputStatesPerSecond,
      "states/s",
    ...
    achieved_logical_error_probability: derived(
      core.achievedLogicalError,
      "probability",
      core.achievedLogicalError === null
        ? notComputable(core.infeasibilityReason as string)
