...
import { INTELLIGENCE_ESTIMATOR, INTELLIGENCE_ESTIMATOR_VERSION, type ResourceScenario } from "./scenario.js"
import type { QuantumWorkload } from "./workload.js"
import { evaluate, type CoreEvaluation } from "./estimate.js"
 ...
 */
function maxErrorRateForFeasibility(
 ...
 */
function maxErrorRateWithinCapacity(
  workload: QuantumWorkload,
  scenario: ResourceScenario,
  capacity: number,
  ceiling: number,
): number | null {
  const fits = (rate: number): boolean => {
...
interface Refusal {
  threshold: string
  code: ThresholdRefusal
  message: string
}

export function computeAdvantageThresholds(
  workload: QuantumWorkload,
  scenario: ResourceScenario,
  ...
  const refuse = (threshold: string, code: ThresholdRefusal, message: string, unit: string): Quantity => {
  ...
  const bound = (
    value: number,
    unit: string,
    kind: "UPPER_BOUND" | "LOWER_BOUND" | "POINT",
    source: string,
    limitations: string[] = [],
  ): Quantity =>
    quantity({
  ...
  const maxErrorRate = bound(
    feasibilityCeiling.value,
    "probability",
    "UPPER_BOUND",
    feasibilityCeiling.saturated
  ...
  requiredConditions.push(
    `The physical two-qubit error rate must stay below ${feasibilityCeiling.value.toExponential(2)} for any code ` +
      "distance to meet this error budget.",
  )

  const capacity = scenario.hardware.physical_qubit_capacity
  let maxErrorRateWithinCapacityQuantity: Quantity
  if (capacity === null) {
    maxErrorRateWithinCapacityQuantity = refuse(
      "max_physical_error_rate_within_capacity",
      "NO_HARDWARE_CAPACITY",
      "No physical qubit capacity was stated for this hardware model, so there is nothing to fit within. " +
        "A capacity was not assumed.",
      "probability",
    )
  } else {
    ...
    maxErrorRateWithinCapacityQuantity =
      solved === null
        ? refuse(
            "max_physical_error_rate_within_capacity",
            "ESTIMATE_INFEASIBLE",
            `No physical error rate produces a machine within ${capacity.toLocaleString("en-US")} physical qubits. ` +
              "The capacity is the binding constraint, not the error rate.",
            "probability",
          )
        : bound(
            solved,
            "probability",
            "UPPER_BOUND",
            `Highest error rate whose total machine still fits ${capacity.toLocaleString("en-US")} physical qubits.`,
          )
    if (solved !== null) {
      requiredConditions.push(
        `To fit ${capacity.toLocaleString("en-US")} physical qubits, the physical error rate must be at or below ` +
  ...
  const requiredDistance =
    evaluation.codeDistance === null
      ? refuse(
          ...
          "ESTIMATE_INFEASIBLE",
          evaluation.infeasibilityReason ?? "No distance meets the budget under these assumptions.",
          "code distance",
        )
      : bound(
          evaluation.codeDistance,
          "code distance",
          "LOWER_BOUND",
          "Smallest odd distance whose logical error rate meets the per-qubit-per-cycle budget. A smaller " +
  ...
  if (scenario.runtime_target === null) {
    ...
    maxCycleForTarget = bound(
      nanoseconds,
      "ns",
      "UPPER_BOUND",
      `Runtime target divided by the ${Math.round(roundsPerRun).toLocaleString("en-US")} surface-code rounds this ` +
        "run occupies under whichever limiter binds.",
    )
    requiredConditions.push(
      `A surface-code cycle at or below ${nanoseconds.toPrecision(3)} ns would be required to finish within the ` +
        `${scenario.runtime_target} s target.`,
    )
  }

  let minThroughputForTarget: Quantity
  if (scenario.runtime_target === null) {
    minThroughputForTarget = refuse(
      "min_factory_throughput_for_runtime_target",
      "NO_RUNTIME_TARGET",
      "This scenario states no runtime target, so no throughput is required by one.",
      "states/s",
    )
    ...
    minThroughputForTarget = bound(
      0,
      "states/s",
      "LOWER_BOUND",
      "Zero: this circuit consumes no magic states, so no factory throughput is required.",
    )
  } else {
    ...
    minThroughputForTarget = bound(
      ...
      "states/s",
      "LOWER_BOUND",
      `${evaluation.magicStates.toLocaleString("en-US")} magic states divided by the ${scenario.runtime_target} s target.`,
    )
    requiredConditions.push(
      `The magic-state factory must deliver at least ${required.toPrecision(3)} states per second to finish within ` +
        `the ${scenario.runtime_target} s target.`,
    )
  }

  const requiredLogicalCapacity = bound(
    evaluation.occupiedLogical,
    "logical qubits",
    "LOWER_BOUND",
    `Logical patches occupied under ${scenario.layout_model}, including routing space.`,
  )

  const requiredTotalCapacity =
    evaluation.totalPhysical === null
      ? refuse(
          "required_total_physical_qubit_capacity",
          evaluation.feasible ? "FACTORY_NOT_SIZED" : "ESTIMATE_INFEASIBLE",
          evaluation.feasible
            ? `The factory could not be sized, so no total is available. ${evaluation.distillationReason}`
            : (evaluation.infeasibilityReason as string),
          "physical qubits",
        )
      : bound(
          evaluation.totalPhysical,
          "physical qubits",
          "LOWER_BOUND",
          "Algorithm patches, routing space, and factory together. This is the machine, not a component of it.",
        )

  let capacityHeadroom: Quantity
  if (capacity === null) {
    capacityHeadroom = refuse(
      "capacity_headroom",
      "NO_HARDWARE_CAPACITY",
      "No physical qubit capacity was stated for this hardware model.",
      "ratio",
    )
    ...
    capacityHeadroom = refuse(
      "capacity_headroom",
      evaluation.feasible ? "FACTORY_NOT_SIZED" : "ESTIMATE_INFEASIBLE",
      "The total machine size is not available, so it cannot be compared with a capacity.",
      "ratio",
    )
  } else {
    const headroom = capacity / evaluation.totalPhysical
    capacityHeadroom = bound(
      headroom,
      "ratio",
      "POINT",
      `Stated capacity ${capacity.toLocaleString("en-US")} divided by required ` +
        `${Math.round(evaluation.totalPhysical).toLocaleString("en-US")}.`,
      headroom < 1
    ...
    if (headroom < 1) {
      requiredConditions.push(
        `Physical qubit capacity would have to grow by ${(1 / headroom).toPrecision(3)}x to run this workload.`,
      )
    }
  }
  ...
  const noBaseline = (name: string, unit: string): Quantity =>
    refuse(
      name,
      "NO_CLASSICAL_BASELINE",
      `${INSUFFICIENT_ECONOMIC_EVIDENCE}: no classical baseline was supplied, so there is nothing to be faster or ` +
        "cheaper than. No baseline was assumed.",
      unit,
    )

  let maxCycleToBeatClassical: Quantity
  let speedup: Quantity
  if (!hasBaseline) {
    ...
    maxCycleToBeatClassical = refuse(
      ...
      "NO_CLASSICAL_RUNTIME",
      `${INSUFFICIENT_ECONOMIC_EVIDENCE}: the classical baseline records no runtime.`,
      "ns",
    )
    speedup = refuse(
    ...
    speedup = refuse(
      "runtime_speedup_over_classical",
      "ESTIMATE_INFEASIBLE",
      evaluation.infeasibilityReason ?? "The quantum runtime is not available.",
      "ratio",
    )
  } else {
    const nanoseconds = (classicalRuntime * 1e9) / roundsPerRun
    maxCycleToBeatClassical = bound(
      nanoseconds,
      "ns",
      "UPPER_BOUND",
      `Classical runtime ${classicalRuntime} s divided by the ` +
    ...
    requiredConditions.push(
      `A surface-code cycle below ${nanoseconds.toPrecision(3)} ns would be required to beat the supplied ` +
        "classical runtime.",
    )
    speedup = quantity({
  ...
  const noEconomics = (name: string, unit: string): Quantity =>
    refuse(
      name,
      "NO_ECONOMIC_MODEL",
      `${INSUFFICIENT_ECONOMIC_EVIDENCE}: this scenario states no quantum cost model. No price for quantum ` +
        "machine time was assumed, because none exists to look up.",
      unit,
    )

  const noClassicalCost = (name: string, unit: string): Quantity =>
    refuse(
      name,
      "NO_CLASSICAL_COST",
      `${INSUFFICIENT_ECONOMIC_EVIDENCE}: the classical baseline records no monetary cost.`,
      unit,
    )

  let maxMachineCost: Quantity
  let maxQubitSecondCost: Quantity
  let breakEvenRuntime: Quantity
  let breakEvenMachineCost: Quantity
  let projectedCost: Quantity
  let costRatio: Quantity
  ...
  if (!hasBaseline) {
    ...
    const currencyMismatch =
      economics !== null && economics.currency !== currency
        ? [
    ...
    maxMachineCost = bound(
      perSecond,
      `${currency}/s`,
      "UPPER_BOUND",
      `Classical cost ${classicalCost.amount} ${currency} divided by projected quantum runtime ` +
        `${quantumRuntime.toPrecision(3)} s.`,
      currencyMismatch,
    )
    requiredConditions.push(
      `Quantum machine time would have to cost at most ${perSecond.toPrecision(3)} ${currency} per second to ` +
        "match the supplied classical cost.",
    )

    if (evaluation.totalPhysical === null || evaluation.totalPhysical === 0) {
      maxQubitSecondCost = refuse(
        "max_physical_qubit_second_cost",
        "FACTORY_NOT_SIZED",
        `The total machine size is not available, so a per-qubit-second cost cannot be derived. ${evaluation.distillationReason}`,
        `${currency}/qubit/s`,
      )
    } else {
      const perQubitSecond = perSecond / evaluation.totalPhysical
      maxQubitSecondCost = bound(
        perQubitSecond,
        `${currency}/qubit/s`,
        "UPPER_BOUND",
        `Maximum machine-second cost divided by ${Math.round(evaluation.totalPhysical).toLocaleString("en-US")} ` +
          "physical qubits.",
        currencyMismatch,
      )
      requiredConditions.push(
        `That is at most ${perQubitSecond.toExponential(3)} ${currency} per physical qubit per second.`,
      )
    }

    breakEvenMachineCost = bound(
      perSecond,
      `${currency}/s`,
      "POINT",
      "The machine-second cost at which quantum and classical cost the same for this workload.",
      currencyMismatch,
    )
    ...
    if (economics === null) {
      ...
      const message =
        `${INSUFFICIENT_ECONOMIC_EVIDENCE}: the economic model states no machine-second rate, only a ` +
      ...
      if (rate !== null && evaluation.totalPhysical !== null) {
        ...
        costRatio = bound(
          projected / classicalCost.amount,
          "ratio",
          "POINT",
          "Projected quantum cost divided by the supplied classical cost.",
          currencyMismatch,
        )
      } else {
      ...
      breakEvenRuntime = bound(
        classicalCost.amount / rate,
        "s",
        "UPPER_BOUND",
        `Classical cost ${classicalCost.amount} ${currency} divided by the stated machine rate ${rate} ` +
          `${economics.currency}/s: how long a quantum run may take before it costs more than classical.`,
        currencyMismatch,
      )
