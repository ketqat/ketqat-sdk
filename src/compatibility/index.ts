...
export function compareRunCompatibility(
  ...
  if (suite) {
    ...
    for (const metric of requiredMetrics) {
      ...
      if (!hasComparableMetricCoordinate(comparableCoordinates, metric)) {
        reasons.push(
          reason(
            "METRIC_COORDINATE_MISMATCH",
            `Required metric '${metric}' has no overlapping comparable coordinates.`,
            "metric_points",
          ),
        )
      }
  ...
  if (!suite && left.metric_points.length > 0 && right.metric_points.length > 0 && comparableCoordinates.length === 0) {
    reasons.push(
      reason("NO_COMPARABLE_METRIC_COORDINATES", "Runs do not share any comparable metric coordinates.", "metric_points"),
    )
  }
...
function hasComparableMetricCoordinate(coordinates: string[], metric: string): boolean {
