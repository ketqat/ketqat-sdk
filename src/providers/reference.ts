import { HardwareProfileSchema, linearTopology, type HardwareProfile } from "../hardware/profile.js"
import { totalQubits, type QuantumCircuit } from "../circuit/graph.js"
...
const BACKENDS: Record<string, { qubits: number; costPerShot: number | null; quota: number | null }> = {
  "line-5": { qubits: 5, costPerShot: 0.0001, quota: 10_000 },
...
export class ReferenceProviderAdapter implements ProviderAdapter {
  readonly provider = REFERENCE_PROVIDER
  readonly version = "0.1.0"

  async describeBackend(backend: string): Promise<HardwareProfile> {
    const definition = BACKENDS[backend]
    if (!definition) {
      throw new ProviderError(
        `Unknown backend '${backend}'. Known backends: ${Object.keys(BACKENDS).join(", ")}.`,
      )
    }
    return HardwareProfileSchema.parse({
      ...
      source:
        "Reference contract-test adapter. Not an observation of any physical device, and this " +
  ...
  async submit(
    ...
    if (!options.credential) {
      return notRun(
        ...
        "No provider credential was supplied, so nothing was submitted. This is recorded as not " +
    ...
    if (!options.confirmed) {
      return notRun(
        ...
        "The submission was not confirmed. Provider, backend, shot count, estimated cost, and " +
    ...
    if (definition.quota !== null && definition.quota <= 0) {
      return notRun(
        ...
        backend,
        "quota_exhausted",
        `${backend} reports no remaining quota, so nothing was submitted.`,
      )
    }
...
export function resolveProvider(name: string): ProviderAdapter {
  ...
  if (!factory) {
    throw new ProviderError(
      `Unknown provider '${name}'. Available: ${Object.keys(PROVIDER_ADAPTERS).join(", ")}. ` +
