...
export function operationKey(operation: Operation): string {
  const record = operation as unknown as {
    kind: string
    name?: string
    parameters?: ReadonlyArray<number | string>
    qubits?: Array<{ register: string; index: number }>
    qubit?: { register: string; index: number }
    clbit?: { register: string; index: number }
    register?: string
    bit?: number
    equals?: number
    body?: Operation
  ...
  switch (record.kind) {
    ...
    case "barrier":
      return `barrier:${bits}`
    case "conditional":
      return `if(${record.register}${record.bit === undefined ? "" : `[${record.bit}]`}==${record.equals}):${
        record.body ? operationKey(record.body) : "?"
      }`
    default:
...
function commonSubsequence(left: string[], right: string[]): Array<[number, number]> {
  ...
  if (rows * columns > 4_000_000) {
    throw new CircuitDiffError(
      `Aligning ${rows} against ${columns} operations needs a ${rows}x${columns} table, which this ` +
  ...
  for (let row = rows - 1; row >= 0; row -= 1) {
    for (let column = columns - 1; column >= 0; column -= 1) {
      ;(table[row] as number[])[column] =
        left[row] === right[column]
          ? ((table[row + 1] as number[])[column + 1] as number) + 1
          : Math.max(
              (table[row + 1] as number[])[column] as number,
              (table[row] as number[])[column + 1] as number,
            )
    }
  }
  ...
  while (row < rows && column < columns) {
    if (left[row] === right[column]) {
      ...
      ((table[row + 1] as number[])[column] as number) >= ((table[row] as number[])[column + 1] as number)
    ) {
      row += 1
    } else {
      column += 1
 ...
 */
export function diffCircuits(left: QuantumCircuit, right: QuantumCircuit): CircuitDiff {
  ...
  for (const [leftIndex, rightIndex] of matched) {
    while (leftCursor < leftIndex) {
      if (!matchedLeft.has(leftCursor)) {
        entries.push({
          kind: "removed",
          operation: leftOperations[leftCursor] as Operation,
    ...
    while (rightCursor < rightIndex) {
      if (!matchedRight.has(rightCursor)) {
        entries.push({
          kind: "added",
          operation: rightOperations[rightCursor] as Operation,
    ...
    entries.push({
      kind: "unchanged",
      operation: leftOperations[leftIndex] as Operation,
      leftIndex,
      rightIndex,
    })
  ...
  while (leftCursor < leftOperations.length) {
    if (!matchedLeft.has(leftCursor)) {
      entries.push({ kind: "removed", operation: leftOperations[leftCursor] as Operation, leftIndex: leftCursor })
    }
    leftCursor += 1
  }
  while (rightCursor < rightOperations.length) {
    if (!matchedRight.has(rightCursor)) {
      entries.push({ kind: "added", operation: rightOperations[rightCursor] as Operation, rightIndex: rightCursor })
    }
    rightCursor += 1
  }

  const gateName = (operation: Operation): string =>
    (operation as unknown as { kind: string; name?: string }).name ??
    (operation as unknown as { kind: string }).kind

  const names = new Set([...leftOperations, ...rightOperations].map(gateName))
  const gateDeltas: GateCountDelta[] = [...names]
    .map((name) => {
...
export function verifyDiff(
  ...
  const reconstructsLeft =
    fromLeft.length === leftKeys.length && fromLeft.every((key, index) => key === leftKeys[index])
  const reconstructsRight =
    fromRight.length === rightKeys.length && fromRight.every((key, index) => key === rightKeys[index])
  ...
  return {
    ...
    detail: sound
      ...
      : `UNSOUND: ${!reconstructsLeft ? "removed+unchanged does not reproduce the left circuit" : ""}${
          !reconstructsLeft && !reconstructsRight ? "; " : ""
        }${!reconstructsRight ? "unchanged+added does not reproduce the right circuit" : ""}. The diff has ` +
