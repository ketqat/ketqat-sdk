...
import { JobRejectedError, assertWithinLimits, type ExecutionJob, type JobResult } from "./job.js"
...
export async function executeJob(job: ExecutionJob): Promise<JobResult> {
  const startedAt = new Date()
  const start = Date.now()

  const finish = (partial: Omit<JobResult, "started_at" | "finished_at" | "duration_ms" | "schema_version" | "job_id" | "execution_class" | "worker_version">): JobResult => ({
    schema_version: "0.1",
    job_id: job.job_id,
  ...
  try {
    ...
    const message =
      error instanceof Qasm3ParseError
...
async function runOperation(job: ExecutionJob): Promise<Record<string, unknown>> {
  const { parameters, limits } = job

  const parse = (qasm: string) => {
    const parsed = parseQasm3(qasm)
    const qubits = totalQubits(parsed.circuit)
    if (qubits > limits.max_qubits) {
      throw new JobRejectedError(
        `Circuit uses ${qubits} qubits, above this job's limit of ${limits.max_qubits}.`,
      )
    }
    return parsed
  }
 ...
 */
export function enforceResultSize(result: JobResult, maxBytes: number): JobResult {
  ...
  return {
    ...
    output: undefined,
    error:
      `Result exceeded the ${maxBytes}-byte limit for this job. It is reported as failed rather than ` +
