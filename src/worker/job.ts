...
export const JobLimitsSchema = z.object({
  ...
  max_shots: z.number().int().positive().max(1_000_000).default(100_000),
  /** Cap on result size, so a job cannot exhaust storage by succeeding. */
  max_result_bytes: z.number().int().positive().max(50_000_000).default(5_000_000),
})
...
const CircuitParameters = z.object({
  ...
  qasm: z.string().min(1).max(1_000_000),
})
...
export const JobParametersSchema = z.discriminatedUnion("operation", [
  ...
  z.object({
    ...
    left_qasm: z.string().min(1).max(1_000_000),
    right_qasm: z.string().min(1).max(1_000_000),
 ...
 */
export function validateJob(input: unknown): ExecutionJob {
  assertNoForbiddenFields(input, "")
  const parsed = ExecutionJobSchema.safeParse(input)
  if (!parsed.success) {
    throw new JobRejectedError(
      `Job failed validation: ${parsed.error.issues
...
function assertNoForbiddenFields(value: unknown, path: string): void {
  ...
  for (const [key, nested] of Object.entries(value as Record<string, unknown>)) {
    if ((FORBIDDEN_JOB_FIELDS as readonly string[]).includes(key.toLowerCase())) {
      throw new JobRejectedError(
        `Job contains a forbidden field '${path ? `${path}.` : ""}${key}'. Jobs name an approved ` +
...
export function assertWithinLimits(job: ExecutionJob): void {
  const { limits, parameters } = job
  if ("shots" in parameters && parameters.shots !== undefined && parameters.shots > limits.max_shots) {
    throw new JobRejectedError(
      `Requested ${parameters.shots} shots, above this job's limit of ${limits.max_shots}.`,
    )
  }
}
