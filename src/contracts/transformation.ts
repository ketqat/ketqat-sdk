import { z } from "zod"
import {
  EquivalenceLevelSchema,
  LossReportEntrySchema,
  type LossReportEntry,
} from "./common.js"
  ...
  .superRefine((evidence, context) => {
    if (evidence.level === "FAILED" && evidence.counterexample === undefined) {
      context.addIssue({
        ...
        path: ["counterexample"],
        message:
          "FAILED requires a counterexample. Without one the correct level is INCONCLUSIVE: " +
...
export function hasSemanticLoss(entries: LossReportEntry[]): boolean {
 ...
 */
export function chainHasSemanticLoss(chain: TransformationChain): boolean {
