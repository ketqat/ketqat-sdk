...
const ResultRowShape = (name: string): StudyShape =>
  declareShape(name, [field("label", "RECORD_ONLY"), field("node_hash", "SEMANTIC")])

const FigureShape: StudyShape = declareShape("study.figure", [
...
const StudyTaskShapeDecl: StudyShape = declareShape("study_task", [
  ...derivedHeaderFields("content_hash"),
  field("study_ref", "RECORD_ONLY"),
  field("kind", "SEMANTIC"),
...
export function flattenSchemaFields(schema: z.ZodTypeAny): readonly string[] {
  ...
  const visit = (current: z.ZodTypeAny, prefix: string, depth: number): void => {
    ...
    const def = inner._def as { typeName?: string; shape?: () => z.ZodRawShape; type?: z.ZodTypeAny }
