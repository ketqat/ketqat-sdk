...
const BOM_UTF8 = Object.freeze([0xef, 0xbb, 0xbf] as const)
...
function decodeStrictUtf8(bytes: Uint8Array): string {
  try {
    ...
    refuse(
      "INVALID_UTF8",
      "the file is not valid UTF-8. JSON text is UTF-8 (RFC 8259 §8.1), and a decoder that substituted U+FFFD " +
...
function assertNoDuplicateKeys(text: string): void {
  const stack: { readonly kind: "object" | "array"; readonly names: Set<string> }[] = []
  let index = 0
  ...
  while (index < length) {
    ...
    if (char === '"') {
      ...
      if (top !== undefined && top.kind === "object" && isPropertyName(text, index)) {
        if (top.names.has(value)) {
          refuse(
            "DUPLICATE_PROPERTY",
            `the object contains the property ${JSON.stringify(value)} twice, at byte offset ${start}. RFC 8259 ` +
    ...
    if (char === "{") {
      stack.push({ kind: "object", names: new Set<string>() })
      index += 1
      continue
    }
    if (char === "[") {
      stack.push({ kind: "array", names: new Set<string>() })
      index += 1
      continue
    }
    if (char === "}" || char === "]") {
 ...
 */
function checkNumberLiteral(text: string, start: number): number {
  let index = start
  ...
  if (isInteger && integerDigits.replace(/^0+(?=\d)/, "").length >= 16) {
    ...
    if (magnitude > BigInt(Number.MAX_SAFE_INTEGER)) {
      refuse(
        "UNSAFE_INTEGER",
        `the integer literal at byte offset ${start} is outside ±${Number.MAX_SAFE_INTEGER}. Python holds it ` +
 ...
 */
function isPropertyName(text: string, index: number): boolean {
  for (let scan = index; scan < text.length; scan += 1) {
    const char = text[scan] as string
    if (char === " " || char === "\t" || char === "\n" || char === "\r") continue
    return char === ":"
  }
  return false
 ...
 */
function assertNoUnpairedSurrogates(decoded: string, offset: number): void {
  for (let index = 0; index < decoded.length; index += 1) {
    ...
    if (unit >= 0xdc00 && unit <= 0xdfff) refuseSurrogate(unit, offset, "low")
  }
}

function refuseSurrogate(unit: number, offset: number, half: "high" | "low"): never {
  refuse(
    "LONE_SURROGATE",
    `the string starting at byte offset ${offset} carries a lone ${half} surrogate ` +
 ...
 * before parse.
 */
export function readStudyFileBytes(bytes: Uint8Array): StudyFileReading {
  if (!(bytes instanceof Uint8Array)) {
    refuse(
      "NOT_JSON_VALUE",
      "file verification is defined over raw bytes and takes a Uint8Array. A string has already been decoded by " +
  ...
  if (
    ...
    refuse(
      "BYTE_ORDER_MARK",
      "the file begins with a UTF-8 byte order mark. RFC 8259 §8.1 says an implementation MUST NOT add one and " +
  ...
  try {
    ...
    refuse("INVALID_JSON", `the file is not JSON: ${(error as Error).message}`)
  }
  return Object.freeze({ value: value as JsonValue, text })
}
