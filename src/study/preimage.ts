...
import { STUDY_HASH_PURPOSES, type StudyHashPurpose } from "./projection.js"
...
function assertComponent(name: string, value: unknown): asserts value is string {
  if (typeof value !== "string" || value.length === 0) {
    refuse(
      "MISSING_HEADER_COMPONENT",
      `the preimage header needs a ${name}, and this record does not supply one. Nothing is inferred: a study ` +
  ...
  if (value.length > MAX_COMPONENT_LENGTH) {
    refuse(
      ...
      `the ${name} header component is longer than ${MAX_COMPONENT_LENGTH} characters.`,
    )
  }
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (code < 0x21 || code > 0x7e) {
      refuse(
        "INVALID_HEADER_COMPONENT",
        `the ${name} header component contains a character outside printable ASCII at index ${index}. The header ` +
...
const purposes = new Set<string>(STUDY_HASH_PURPOSES)

function assertPurpose(value: unknown): asserts value is StudyHashPurpose {
  assertComponent("hash purpose", value)
  if (!purposes.has(value)) {
    refuse(
      "INVALID_HEADER_COMPONENT",
      `${JSON.stringify(value)} is not a hash purpose. Known purposes: ${STUDY_HASH_PURPOSES.join(", ")}. ` +
...
const knownRulesIds = new Set<string>([STUDY_HASH_RULES_ID])

function assertKnownRulesId(value: string): void {
  if (knownRulesIds.has(value)) return
  refuse(
    "UNKNOWN_HASH_RULES_ID",
    `this build does not know the hash rules id ${JSON.stringify(value)}. Known ids: ` +
 ...
 * in step.
 */
export function buildStudyPreimage(header: StudyPreimageHeader, body: Uint8Array): Uint8Array {
