...
export function isFiniteFloat(value: unknown): value is number {
...
export function isExactIntegerString(value: unknown): value is string {
 ...
 * wrote one was not.
 */
const EXACT_DECIMAL_STRING = new RegExp(
  `^-?(?:0|[1-9][0-9]{0,${EXACT_STRING_MAX_DIGITS - 1}})(?:\\.[0-9]{1,${EXACT_STRING_MAX_DIGITS}})?$`,
)
...
export function isExactDecimalString(value: unknown): value is string {
  ...
  if (!EXACT_DECIMAL_STRING.test(value)) return false
  return !MINUS_ZERO.test(value)
}

export function assertExactIntegerString(value: unknown, path: string): asserts value is string {
  if (isExactIntegerString(value)) return
  refuse(
    "INVALID_EXACT_NUMBER_STRING",
    `${JSON.stringify(value)} is not an exact integer string. This field carries an integer JavaScript may not ` +
...
export function assertExactDecimalString(value: unknown, path: string): asserts value is string {
  if (isExactDecimalString(value)) return
  refuse(
    "INVALID_EXACT_NUMBER_STRING",
    `${JSON.stringify(value)} is not an exact decimal string. This field carries a decimal whose precision must ` +
      "survive, so it is recorded as digits: optional minus, an integer part with no leading zero, and an " +
      "optional fraction. No exponent, and not a signed zero. Trailing zeros are significant and are kept.",
    path,
  )
}
 ...
 * two code paths.
 */
export function exactIntegerStringFromBigInt(value: bigint, path = "(value)"): string {
...
export const ExactIntegerStringSchema = z.string().regex(EXACT_INTEGER_STRING, {
  message:
    "An exact integer string is `0`, or an optional minus and up to 64 digits with a non-zero leading digit: no " +
...
export const ExactDecimalStringSchema = z
  ...
  .regex(EXACT_DECIMAL_STRING, {
    message:
      "An exact decimal string is an optional minus, an integer part with no leading zero, and an optional " +
...
export const STUDY_NUMBER_CONTRACTS: readonly StudyNumberContract[] = Object.freeze([
  Object.freeze({
