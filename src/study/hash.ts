...
import { refuse, STUDY_HASH_LIMITS, type StudyHashLimits } from "./limits.js"
import { buildStudyPreimage, studyHeader } from "./preimage.js"
import { projectStudyShape, type StudyHashPurpose } from "./projection.js"
...
function sha256(preimage: Uint8Array): string {
...
function digestRecord(
  ...
  limits: StudyHashLimits,
): string {
  const entry = studyRecordKind(recordKind)
  const schemaVersion = (record as Record<string, unknown>)["schema_version"]
  const rulesId = (record as Record<string, unknown>)["hash_rules_id"] ?? STUDY_HASH_RULES_ID
  if (typeof schemaVersion !== "string" || schemaVersion.length === 0) {
    refuse(
      "MISSING_HEADER_COMPONENT",
      "a study record must name the schema version it was written against; nothing is inferred. The version is " +
  ...
  if (typeof rulesId !== "string") {
    refuse(
      ...
      `${JSON.stringify(rulesId)} is not a hash rules id. The field must be a string or absent.`,
      "hash_rules_id",
    )
  }
...
export function artifactHash(
  ...
  hashRulesId: string = STUDY_HASH_RULES_ID,
  limits: StudyHashLimits = STUDY_HASH_LIMITS,
): string {
  if (!(bytes instanceof Uint8Array)) {
    refuse(
      "NOT_JSON_VALUE",
      "artifactHash is defined over the literal bytes of a file and takes a Uint8Array. A string would have to " +
  ...
  if (bytes.length > limits.max_canonical_bytes) {
    refuse(
      ...
      `the artifact is ${bytes.length} bytes, past the ${limits.max_canonical_bytes} byte bound.`,
    )
  }
  ...
  return sha256(
    buildStudyPreimage(studyHeader(recordKind, "artifact", schemaVersion, hashRulesId), bytes),
  )
}
 ...
 * digest".
 */
export function verifyStudySelfHash(
  recordKind: string,
  record: object,
  limits: StudyHashLimits = STUDY_HASH_LIMITS,
): StudySelfHashVerification {
  const entry = studyRecordKind(recordKind)
  ...
  const recorded = Object.prototype.hasOwnProperty.call(record, entry.self_hash_field)
    ? (record as Record<string, unknown>)[entry.self_hash_field]
    : undefined
 ...
 */
export function studyCanonicalBody(
  recordKind: string,
  record: object,
  purpose: StudyHashPurpose,
  limits: StudyHashLimits = STUDY_HASH_LIMITS,
