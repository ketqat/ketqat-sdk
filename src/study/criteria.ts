import { z } from "zod"
import { EvidenceClassSchema, type Contract, type EvidenceClass } from "../intelligence/measurement.js"
import { STUDY_DIMENSIONS, studyUnitSchema, type StudyDimension } from "./units.js"
...
export const CRITERION_COMPARATORS: readonly CriterionComparatorRule[] = Object.freeze([
  Object.freeze({ comparator: "LESS_THAN" as const, takes_threshold: true, means: "strictly below the threshold" }),
  Object.freeze({ comparator: "AT_MOST" as const, takes_threshold: true, means: "at or below the threshold" }),
  Object.freeze({ comparator: "EQUAL" as const, takes_threshold: true, means: "equal to the threshold" }),
  Object.freeze({ comparator: "AT_LEAST" as const, takes_threshold: true, means: "at or above the threshold" }),
  Object.freeze({ comparator: "GREATER_THAN" as const, takes_threshold: true, means: "strictly above the threshold" }),
  Object.freeze({ comparator: "EXISTS" as const, takes_threshold: false, means: "the metric resolves to something" }),
  Object.freeze({
    comparator: "DOES_NOT_EXIST" as const,
    takes_threshold: false,
    means: "the metric resolves to nothing",
  }),
  Object.freeze({ comparator: "IS_TRUE" as const, takes_threshold: false, means: "the metric holds" }),
  Object.freeze({ comparator: "IS_FALSE" as const, takes_threshold: false, means: "the metric does not hold" }),
])

const comparatorRules = new Map<string, CriterionComparatorRule>(
  CRITERION_COMPARATORS.map((entry) => [entry.comparator, entry]),
)

export function criterionComparatorTakesThreshold(comparator: CriterionComparator): boolean {
...
const thresholdVariant = (dimension: StudyDimension) =>
  z
    .object({
...
type ThresholdVariant = ReturnType<typeof thresholdVariant>

export const CriterionThresholdSchema: Contract<CriterionThreshold> = z
  .discriminatedUnion(
    "dimension",
    STUDY_DIMENSIONS.map(thresholdVariant) as unknown as [ThresholdVariant, ThresholdVariant],
  )
  .superRefine((threshold, context) => {
    ...
    context.addIssue({
      ...
      message:
        "A threshold is written once: as a finite number, or as exact digits where the value may exceed 2^53. " +
        "Writing both is two spellings of one threshold, and writing neither is a comparison with nothing.",
      path: ["value"],
    })
  }) as unknown as Contract<CriterionThreshold>
...
export const CRITERION_STATUSES: readonly CriterionStatusRule[] = Object.freeze([
  Object.freeze({ status: "PASS" as const, decided: true, means: "the predicate held on evidence of a required kind" }),
  Object.freeze({ status: "FAIL" as const, decided: true, means: "the predicate did not hold" }),
  Object.freeze({ status: "NOT_RUN" as const, decided: false, means: "nothing has been evaluated against it yet" }),
  Object.freeze({
...
export const UNEVALUATED_CRITERION_STATUS: CriterionStatus = "NOT_RUN"
...
export const StudyCriterionSchema: Contract<StudyCriterion> = z
  .object({
  ...
  .superRefine((criterion, context) => {
    ...
    if (!takesThreshold && criterion.threshold !== null) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          `${criterion.comparator} tests whether the metric is there at all, so a threshold beside it is a ` +
    ...
    if (criterion.required_evidence.includes("UNKNOWN")) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "UNKNOWN is the absence of evidence, so a criterion satisfied by it is a criterion satisfied by nothing " +
  ...
  }) as unknown as Contract<StudyCriterion>
 ...
 */
export function duplicateCriterionIds(criteria: readonly { criterion_id: string }[]): readonly string[] {
  const seen = new Set<string>()
  const duplicates = new Set<string>()
  for (const criterion of criteria) {
    if (seen.has(criterion.criterion_id)) duplicates.add(criterion.criterion_id)
