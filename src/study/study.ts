...
export const StudySchema: Contract<Study> = z.object({
  ...
  .superRefine((event, context) => {
    ...
    if (event.from_status !== null && !isValidStudyTransition(event.from_status, event.to_status)) {
      ...
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          `A study cannot move from ${event.from_status} to ${event.to_status}. ` +
          (reachable.length === 0
    ...
    if (event.to_status === "RUNNING" && event.plan_ref === null) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "An event that starts a run must name the confirmed plan revision it runs. A run with no plan reference " +
  ...
  if (previous !== null && previous.study_ref !== study.content_hash) {
    return {
      ...
      refusal: {
        ...
        message:
          `The trail belongs to study ${previous.study_ref}, not to ${study.content_hash}. ` +
  ...
  if (!isValidStudyTransition(fromStatus, input.toStatus)) {
    ...
    return {
      ...
      refusal: {
        ...
        message:
          `A study at ${fromStatus ?? "no status yet"} cannot move to ${input.toStatus}. ` +
          (reachable.length === 0
 ...
 */
export function verifyStudyEventChain(events: readonly StudyEvent[]): {
  valid: boolean
