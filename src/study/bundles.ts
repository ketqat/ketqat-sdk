...
export const EmbeddedBundleSchema: Contract<EmbeddedBundle> = z
  .object({
    ...
    base64: z.string().max(STUDY_PACKAGE_LIMITS.max_embedded_bundle_bytes * 2).regex(STRICT_BASE64, {
      message:
        "An embedded bundle is standard base64 with no line breaks and no URL-safe substitutions. Two spellings " +
...
export const BundleRefSchema: Contract<BundleRef> = z
  .object({
...
export const BundleFieldRefSchema: Contract<BundleFieldRef> = z
  .object({
    bundle_hash: ContentHashSchema,
    field_path: z.string().min(1).max(256).regex(BUNDLE_FIELD_PATH, {
      message:
        "A bundle field path is properties separated by dots and array indices in brackets. Wildcards and filters " +
 ...
 */
export const BUNDLE_DERIVED_RECORD_KINDS: readonly string[] = Object.freeze([
...
const bundleDerivedKinds = new Set<string>(BUNDLE_DERIVED_RECORD_KINDS)

export function isBundleDerivedRecordKind(kind: string): boolean {
 ...
 */
export function decodeEmbeddedBundle(
  embedded: EmbeddedBundle,
  schemaVersion: string,
  path: string,
): { ok: true; document: unknown } | { ok: false; findings: StudyFinding[] } {
  ...
  if (overSize !== null) return { ok: false, findings: [overSize] }
    ...
    return {
      ...
      findings: [
        finding(
          ...
          `${path}.byte_size`,
          `The embedded bundle decodes to ${bytes.length} bytes and the record says ${embedded.byte_size}. Two ` +
    ...
    return {
      ...
      findings: [
        finding(
          "BUNDLE_HASH_MISMATCH",
          `${path}.content_hash`,
          `The embedded bundle's bytes hash to ${digest} and the record claims ${embedded.content_hash}. The ` +
  ...
  try {
    return { ok: true, document: JSON.parse(new TextDecoder().decode(bytes)) as unknown }
  } catch (error) {
    return {
      ok: false,
      findings: [
        finding(
          "BUNDLE_UNRESOLVED",
          `${path}.base64`,
          `The embedded bundle's bytes are not JSON: ${error instanceof Error ? error.message : String(error)}. ` +
 ...
 */
export function resolveBundleField(document: unknown, fieldPath: string): unknown {
  let current: unknown = document
  for (const part of fieldPath.split(".")) {
    const match = /^([A-Za-z_][A-Za-z0-9_]*)((?:\[[0-9]+\])*)$/.exec(part)
    if (match === null) return undefined
    if (current === null || typeof current !== "object") return undefined
    if (!Object.prototype.hasOwnProperty.call(current, match[1] as string)) return undefined
...
export function resolveBundles(
  refs: readonly BundleRef[],
  schemaVersion: string,
  supplied: BundleResolver = new Map(),
  distribution: "ONLINE" | "OFFLINE_EXPORT" = "ONLINE",
  section = "bundle_refs",
  /**
    ...
    if (ref.embedded !== null) {
      ...
      findings.push(
        finding(
          ...
          `${path}.embedded`,
          `This package calls itself an offline export and references bundle ${ref.reproducibility_hash} without ` +
        ...
        if (requireResolution) {
          findings.push(
            finding(
              ...
              `Bundle ${ref.reproducibility_hash} is neither carried by this package nor supplied to the ` +
    ...
    if (kind !== ref.bundle_kind) {
      findings.push(
        finding(
          ...
          `${path}.bundle_kind`,
          `The reference says ${JSON.stringify(ref.bundle_kind)} and the document says ${JSON.stringify(kind)}. ` +
    ...
    if (recordedHash !== ref.reproducibility_hash) {
      findings.push(
        finding(
          "BUNDLE_HASH_MISMATCH",
          `${path}.reproducibility_hash`,
          `The reference names ${ref.reproducibility_hash} and the document carries ` +
    ...
    if (!verification.hash_matches) {
      ...
      findings.push(
        finding(
          "BUNDLE_HASH_MISMATCH",
          `${path}.reproducibility_hash`,
          `The bundle's own contents canonicalize to ${verification.expected_hash} and it claims ` +
    ...
    if (!verification.estimates_match || !verification.decision_matches) {
      ...
      findings.push(
        finding(
          ...
          path,
          "This bundle's stored estimates or decisions are not what its own inputs produce, so a number this " +
 ...
 * claim in.
 */
export function bundleFieldFindings(
  fields: readonly BundleFieldRef[],
  ...
  fields.forEach((field, index) => {
    ...
    if (!declared.has(field.bundle_hash)) {
      findings.push(
        finding(
          "BUNDLE_UNRESOLVED",
          `${path}[${index}].bundle_hash`,
          `This claim reads bundle ${field.bundle_hash}, and the package does not reference it in bundle_refs. ` +
    ...
    if (resolveBundleField(document, field.field_path) !== undefined) return
    findings.push(
      finding(
        "BUNDLE_FIELD_UNRESOLVED",
        `${path}[${index}].field_path`,
        `The bundle carries nothing at ${JSON.stringify(field.field_path)}. A claim naming a field that is not ` +
