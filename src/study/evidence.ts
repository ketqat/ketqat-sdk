...
export const EVIDENCE_RECORD_KINDS: readonly EvidenceRecordKind[] = Object.freeze([
...
export const EvidenceReferenceSchema: Contract<EvidenceReference> = z
  .object({
  ...
  .superRefine((reference, context) => {
    if (reference.hash === null && reference.record_slug === null) {
      context.addIssue({
        ...
        message:
          "A reference must name what it points at, by content hash or by registry slug. A reference to nothing " +
...
export const ClaimValueRefSchema: Contract<ClaimValueRef> = z
  .object({
  ...
  .superRefine((reference, context) => {
    if (reference.kind === "result_field" && reference.field_path === null) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "A result_field reference must say which field of the result carries the number. Without one it names a " +
    ...
    if (reference.kind === "value_node" && reference.field_path !== null) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "A value_node reference reads the node's own value, so there is no field to name. A path beside it would " +
...
export const ClaimStatementSchema: Contract<ClaimStatement> = z
  .object({
    /**
...
type EvidencePayloadKey = "claim" | "quantity" | "reference" | "citation"

const EVIDENCE_PAYLOAD_KEYS: readonly EvidencePayloadKey[] = ["claim", "quantity", "reference", "citation"]
 ...
 */
const NODE_PAYLOAD_KEY: Record<EvidenceNodeKind, EvidencePayloadKey> = {
  claim: "claim",
  quantity: "quantity",
  assumption: "quantity",
  input: "reference",
 ...
 */
const NODE_REFERENCE_RECORD_KIND: Partial<Record<EvidenceNodeKind, EvidenceRecordKind>> = {
  capsule_ref: "execution_capsule",
}

export const EvidenceNodeSchema: Contract<EvidenceNode> = z
  .object({
  ...
  .superRefine((node, context) => {
    ...
    for (const key of EVIDENCE_PAYLOAD_KEYS) {
      ...
      if (key === required && !present) {
        context.addIssue({
          code: z.ZodIssueCode.custom,
          message:
            `A ${node.kind} node must carry its ${key} block. The payload is what the node says; a node of this ` +
      ...
      if (key !== required && present) {
        context.addIssue({
          code: z.ZodIssueCode.custom,
          message:
            `A ${node.kind} node must leave ${key} null; a ${node.kind} node carries ${required}. One kind, one ` +
    ...
    if (node.kind === "assumption" && node.quantity !== null && !isKnown(node.quantity)) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "An assumption must state the value it assumes. An UNKNOWN belongs in a quantity node, which is the kind " +
    ...
    if (pinned !== undefined && node.reference !== null && node.reference.record_kind !== pinned) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          `A ${node.kind} node references a ${pinned}; this one names a ${node.reference.record_kind}. The node ` +
...
export const EvidenceEdgeSchema: Contract<EvidenceEdge> = z
  .object({
    schema_version: z.string().min(1),
    hash_rules_id: z.literal(STUDY_HASH_RULES_ID),
    /** The study this record belongs to, by its stable id: a rename does not break this reference. */
  ...
  .superRefine((edge, context) => {
    if (edge.from_node_hash === edge.to_node_hash) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "An edge must join two different nodes. A node that supports, contradicts or supersedes itself would " +
...
interface EvidenceEdgeRow {
  readonly edge_kind: EvidenceEdgeKind
  readonly from_kind: EvidenceNodeKind
  readonly to_kinds: readonly EvidenceNodeKind[]
  readonly why: string
}

const ASSERTION_KINDS: readonly EvidenceNodeKind[] = ["claim", "quantity", "result", "source"]

const EVIDENCE_EDGE_ROWS: readonly EvidenceEdgeRow[] = [
  {
  ...
  ...ASSERTION_KINDS.map((fromKind) => ({
    edge_kind: "contradicts" as const,
    from_kind: fromKind,
...
export const EVIDENCE_EDGE_MATRIX: readonly EvidenceEdgeRule[] = Object.freeze(
  EVIDENCE_EDGE_ROWS.flatMap((row) =>
    row.to_kinds.map((toKind) =>
      Object.freeze({
 ...
 */
const permittedEdges = new Set(
  EVIDENCE_EDGE_MATRIX.map((rule) => `${rule.from_kind}\u0000${rule.edge_kind}\u0000${rule.to_kind}`),
)

/** Whether this family defines a relation between these two kinds of node. */
export function isEvidenceEdgePermitted(
...
export const EVIDENCE_GROUND_RULES: readonly EvidenceGroundRule[] = Object.freeze([
  Object.freeze({
    node_kind: "claim" as const,
    grounds: "never" as const,
    why: "a claim is the thing being grounded; a chain of them establishes agreement, not evidence",
  }),
  Object.freeze({
    node_kind: "quantity" as const,
    grounds: "when_unknown" as const,
    why: "a reported number is grounded by what produced it, unless it is the explicit absence of one",
  }),
  Object.freeze({
    node_kind: "assumption" as const,
    grounds: "always" as const,
  ...
  Object.freeze({
    node_kind: "dataset_ref" as const,
    grounds: "always" as const,
    why: "a dataset version is material that exists independently of this study",
  }),
  Object.freeze({
    node_kind: "snapshot_ref" as const,
    grounds: "always" as const,
    why: "a snapshot is a dated observation of parameters, which is a dataset version by another name",
  }),
])

const groundRuleByKind = new Map<EvidenceNodeKind, EvidenceGroundRule>(
  EVIDENCE_GROUND_RULES.map((rule) => [rule.node_kind, rule]),
)

/** The grounding rule for one node kind. */
export function evidenceGroundRule(kind: EvidenceNodeKind): EvidenceGroundRule {
...
export function groundsAChain(node: EvidenceNode): boolean {
  const rule = evidenceGroundRule(node.kind)
  if (rule.grounds === "always") return true
  if (rule.grounds === "never") return false
...
export function supersessionChain(
  ...
  while (index.has(current) && !seen.has(current)) {
    seen.add(current)
    chain.push(index.get(current) as EvidenceNode)
 ...
 */
export function contradictionSet(
  nodes: readonly EvidenceNode[],
  edges: readonly EvidenceEdge[],
  nodeHash: string,
): EvidenceNode[] {
  ...
  const superseded = new Set(
    edges.filter((edge) => edge.kind === "supersedes").map((edge) => edge.to_node_hash),
  )
  const found: EvidenceNode[] = []
  const seen = new Set<string>()
  for (const edge of edges) {
    if (edge.kind !== "contradicts") continue
    const other =
      edge.from_node_hash === nodeHash
        ? edge.to_node_hash
        : edge.to_node_hash === nodeHash
          ? edge.from_node_hash
          : null
    if (other === null || seen.has(other) || superseded.has(other)) continue
 ...
 * a finding.
 */
const PROVENANCE_EDGE_KINDS: readonly EvidenceEdgeKind[] = ["derived_from", "used_input"]

interface ProvenanceStep {
  readonly hash: string
  readonly via: EvidenceEdge
}

function provenanceParents(
  node: EvidenceNode,
  edges: readonly EvidenceEdge[],
): ProvenanceStep[] {
  if (node.kind === "claim") {
  ...
  return edges
    .filter(
      (edge) =>
        PROVENANCE_EDGE_KINDS.includes(edge.kind) && edge.from_node_hash === node.content_hash,
    )
 ...
 */
export function verifyProvenanceClosure(
  nodes: readonly EvidenceNode[],
  edges: readonly EvidenceEdge[],
  claimNodeHash: string,
): ClaimProvenanceClosure {
  const index = nodesByHash(nodes)
  ...
  if (claim === undefined) {
    refusals.push({
      ...
      message:
        "The graph does not carry the claim node this hash names, so nothing can be said about what grounds it.",
    })
  ...
  if (claim.kind !== "claim") {
    refusals.push({
      ...
      message:
        `Closure was asked about node ${claimNodeHash}, which is a ${claim.kind} node rather than a claim. Only a ` +
  ...
  const groundedByHash = new Map<string, boolean>()
  const reportedBranches = new Set<string>()
  const reportedCycles = new Set<string>()

  const walk = (hash: string, path: readonly string[]): boolean => {
    if (path.includes(hash)) {
      ...
      if (!reportedCycles.has(key)) {
        ...
        refusals.push({
          ...
          message:
            `Following what this claim rests on returns to node ${hash}. Provenance that comes back to where it ` +
    ...
    if (parents.length === 0) {
      if (!reportedBranches.has(hash) && hash !== claimNodeHash) {
        ...
        refusals.push({
          ...
          message:
            `The ${node.kind} node '${node.label}' is offered in support of this claim and rests on nothing: no ` +
    ...
    for (const parent of parents) {
      ...
      if (walk(parent.hash, nextPath)) grounded = true
    }
  ...
  if (supporting.length === 0) {
    refusals.push({
      ...
      message:
        "This claim has no evidence node supporting it. A claim that cannot be walked back to a result, a source " +
  ...
  if (!grounded) {
    refusals.push({
      ...
      message:
        "Every path out of this claim ends in another claim, in a number nobody says where they got, or in " +
...
export function resolveClaimEvidence(
  ...
  if (claim === null) {
    refusals.push({
      ...
      code: "EVIDENCE_NODE_UNRESOLVED",
      message:
        "The graph does not carry the claim node this hash names, so nothing can be said about what supports it.",
    })
    return {
      claim: null,
      supporting: [],
  ...
  for (const edge of edges) {
    if (edge.kind !== "supports" || edge.to_node_hash !== claimNodeHash) continue
    ...
    if (!source) {
      refusals.push({
        ...
        message:
          `A supports edge names ${edge.from_node_hash} as its evidence, and the graph does not carry that node. ` +
...
export const ClaimMapEntrySchema: Contract<ClaimMapEntry> = z
  .object({
 ...
 */
function assertsSupport(edge: EvidenceEdge, evidenceHash: string, claimHash: string): boolean {
  return (
    edge.kind === "supports" &&
...
function assertsContradiction(edge: EvidenceEdge, otherHash: string, claimHash: string): boolean {
  if (edge.kind !== "contradicts") return false
  return (
    (edge.from_node_hash === otherHash && edge.to_node_hash === claimHash) ||
 ...
 * anything.
 */
export function verifyClaimMap(
  entries: readonly ClaimMapEntry[],
  nodes: readonly EvidenceNode[],
  edges: readonly EvidenceEdge[],
): StudyRefusal[] {
  const refusals: StudyRefusal[] = []
  ...
  for (const entry of entries) {
    ...
    if (seenClaims.has(entry.claim_node_hash)) {
      refusals.push({
        ...
        message:
          "Two entries in this claim map name the same claim, so the map has two answers to what that claim rests " +
    ...
    if (claim === undefined) {
      refusals.push({
        ...
        code: "EVIDENCE_NODE_UNRESOLVED",
        message:
          "The claim map names a claim node the graph does not carry. The map would resolve to nothing for a " +
    ...
    if (claim.kind !== "claim") {
      refusals.push({
        ...
        code: "EVIDENCE_NODE_KIND_MISMATCH",
        message:
          `The claim map has an entry for a ${claim.kind} node. The map says what a sentence rests on, and a ` +
    ...
    for (const hash of entry.supporting_edge_hashes) {
      if (edgesByHash.has(hash)) continue
      refusals.push({
        subject,
        code: "EVIDENCE_EDGE_ENDPOINT_UNRESOLVED",
        message:
          `The claim map cites edge ${hash}, and no edge in this graph has that hash. The relation it names cannot ` +
    ...
    for (const hash of entry.contradicting_edge_hashes) {
      ...
      refusals.push({
        ...
        code: "EVIDENCE_EDGE_ENDPOINT_UNRESOLVED",
        message:
          `The claim map cites edge ${hash} as a contradiction, and no edge in this graph has that hash.`,
      })
    }
    ...
    for (const hash of entry.supporting_node_hashes) {
      ...
      if (!index.has(hash)) {
        refusals.push({
          ...
          code: "EVIDENCE_NODE_UNRESOLVED",
          message:
            `The claim is said to rest on node ${hash}, and the graph does not carry it. Evidence that cannot be ` +
      ...
      if (citedSupport.some((edge) => assertsSupport(edge, hash, entry.claim_node_hash))) continue
      if (edges.some((edge) => assertsContradiction(edge, hash, entry.claim_node_hash))) {
        refusals.push({
          subject,
          code: "CLAIM_EVIDENCE_CONTRADICTS",
          message:
            `Node ${hash} is listed as support for this claim, and the edge joining them says it contradicts it. ` +
      ...
      refusals.push({
        ...
        message:
          `The claim is said to rest on node ${hash}, and none of the edges this entry names joins the two. The ` +
    ...
    for (const hash of entry.contradicting_node_hashes) {
      if (!index.has(hash)) {
        refusals.push({
          ...
          code: "EVIDENCE_NODE_UNRESOLVED",
          message:
            `The claim is said to be contradicted by node ${hash}, and the graph does not carry it. An objection a ` +
      ...
      refusals.push({
        ...
        code: "CLAIM_EVIDENCE_UNLINKED",
        message:
          `Node ${hash} is listed as contradicting this claim, and none of the edges this entry names says so. A ` +
  ...
  for (const node of nodes) {
    ...
    if (seenClaims.has(node.content_hash)) continue
    refusals.push({
      subject: node.label,
      code: "CLAIM_WITHOUT_EVIDENCE_NODE",
      message:
        "This claim node appears in the graph with no entry in the claim map, so nothing states what it rests on. " +
 ...
 * establish.
 */
export function verifyEvidenceGraph(
  nodes: readonly EvidenceNode[],
  edges: readonly EvidenceEdge[],
  options: EvidenceGraphOptions = {},
): EvidenceGraphVerification {
  ...
  for (const node of nodes) {
    ...
    if (expected !== node.content_hash) {
      hashesMatch = false
      problems.push(
        `Node '${node.label}' claims content hash ${node.content_hash} and its own contents canonicalize to ${expected}.`,
      )
    }
    if (index.has(node.content_hash)) {
      refusals.push({
        ...
        message:
          `Node hash ${node.content_hash} appears twice. A node is identified by its content, so one list entry is ` +
  ...
  for (const edge of edges) {
    ...
    if (expected !== edge.content_hash) {
      hashesMatch = false
      problems.push(
        `Edge ${edge.content_hash} (${edge.kind}) canonicalizes to ${expected}; its recorded hash does not match ` +
          "what it says.",
      )
    }
    if (edgeHashes.has(edge.content_hash)) {
      refusals.push({
        ...
        message:
          `Edge hash ${edge.content_hash} appears twice. An edge is identified by its content for the same reason a ` +
  ...
  if (studyRefs.size > 1) {
    refusals.push({
      ...
      message:
        "The nodes and edges in this graph name more than one study. A graph is one study's account of what it " +
  ...
  if (audience === "public") {
    for (const node of nodes) {
      ...
      refusals.push({
        ...
        message:
          "This node is marked PRIVATE and the graph is being verified for a public audience. Visibility is inside " +
  ...
  for (const edge of edges) {
    ...
    if (edge.from_node_hash === edge.to_node_hash) {
      ...
      refusals.push({
        ...
        message:
          "An edge joins one node to itself. It asserts nothing, and it closes every traversal in this module " +
          "into a loop.",
      })
    ...
    if (seenRelations.has(relation)) {
      refusals.push({
        ...
        code: "EVIDENCE_EDGE_DUPLICATE",
        message:
          `The same ${edge.kind} relation between the same two nodes is asserted more than once. Two edges with ` +
    ...
    if (isEvidenceEdgePermitted(from.kind, edge.kind, to.kind)) continue
    edgesPermitted = false
    refusals.push({
      subject: edge.content_hash,
      code: "EVIDENCE_EDGE_NOT_PERMITTED",
      message:
        `This family defines no relation '${from.kind} ${edge.kind} ${to.kind}'. An edge whose triple is not in ` +
    ...
    problems.push(
      `Edge ${edge.content_hash} asserts '${from.kind} ${edge.kind} ${to.kind}', which the matrix does not define.`,
    )
  }
  ...
  for (const node of nodes) {
    ...
    if (!closure.grounded) claimsGrounded = false
    refusals.push(...closure.refusals)
  }
...
function cycleRefusals(
  ...
  const visit = (hash: string): void => {
    ...
    if (onPath.has(hash)) {
      if (!reported.has(hash)) {
        ...
        refusals.push({
          ...
          code: "EVIDENCE_GRAPH_CYCLE",
          message:
            `Following ${kind} edges from node ${hash} returns to it. That relation runs one way through time: a ` +
    ...
    onPath.add(hash)
    for (const next of outgoing.get(hash) ?? []) visit(next)
    onPath.delete(hash)
    settled.add(hash)
  }

  for (const node of nodes) visit(node.content_hash)
  ...
  for (const hash of outgoing.keys()) visit(hash)
  return refusals
}
 ...
 */
function supersessionForkRefusals(
  index: ReadonlyMap<string, EvidenceNode>,
  edges: readonly EvidenceEdge[],
): StudyRefusal[] {
  ...
  for (const [hash, count] of replacements) {
    ...
    refusals.push({
      ...
      message:
        `${count} nodes supersede node ${hash}. A reader following supersession forward is asking which record ` +
 ...
 */
function referenceAgreementRefusals(
  nodes: readonly EvidenceNode[],
  index: ReadonlyMap<string, EvidenceNode>,
): StudyRefusal[] {
  const refusals: StudyRefusal[] = []
  const kindByHash = new Map<string, string>()
  const slugByHash = new Map<string, string>()
  const hashBySlug = new Map<string, string>()

  const references: { node: EvidenceNode; reference: EvidenceReference }[] = []
  for (const node of nodes) {
    if (node.reference !== null) references.push({ node, reference: node.reference })
    if (node.claim !== null) references.push({ node, reference: node.claim.subject_ref })
  }
  ...
  for (const { node, reference } of references) {
    ...
    if (hash !== null && index.has(hash) && recordKind !== "evidence_node") {
      refusals.push({
        ...
        code: "EVIDENCE_NODE_KIND_MISMATCH",
        message:
          `A reference names ${hash} as a ${recordKind}, and that hash is an evidence node of this graph. The ` +
    ...
    if (hash !== null) {
      ...
      if (seenKind !== undefined && seenKind !== recordKind) {
        refusals.push({
          ...
          message:
            `Record ${hash} is referenced as a ${seenKind} elsewhere in this graph and as a ${recordKind} here. ` +
    ...
    if (hash !== null && slug !== null) {
      ...
      if (seenSlug !== undefined && seenSlug !== slug) {
        refusals.push({
          ...
          code: "EVIDENCE_REFERENCE_DISAGREES",
          message:
            `Record ${hash} is called '${seenSlug}' elsewhere in this graph and '${slug}' here. A slug may be ` +
      ...
      if (seenHash !== undefined && seenHash !== hash) {
        refusals.push({
          ...
          code: "EVIDENCE_REFERENCE_DISAGREES",
          message:
            `The ${recordKind} '${slug}' is referenced as ${seenHash} elsewhere in this graph and as ${hash} here. ` +
 ...
 */
export function verifyClaimValues(
  nodes: readonly EvidenceNode[],
  index: ReadonlyMap<string, EvidenceNode> = nodesByHash(nodes),
): StudyRefusal[] {
  const refusals: StudyRefusal[] = []
  for (const node of nodes) {
    ...
    if (target === undefined) {
      refusals.push({
        ...
        code: "EVIDENCE_NODE_UNRESOLVED",
        message:
          `This claim reads its value from node ${valueRef.node_hash}, and the graph does not carry it. The ` +
          "sentence would render with no number a reader could check, or with one a renderer supplied.",
      })
      continue
    }
    if (valueRef.kind === "result_field") {
      if (target.kind === "result") continue
      refusals.push({
        subject: node.label,
        code: "EVIDENCE_NODE_KIND_MISMATCH",
        message:
          `This claim reads a field of a result, and node ${valueRef.node_hash} is a ${target.kind} node. Only a ` +
    ...
    if (target.kind !== "quantity" && target.kind !== "assumption") {
      refusals.push({
        ...
        code: "EVIDENCE_NODE_KIND_MISMATCH",
        message:
          `This claim reads its value from a ${target.kind} node, which carries no value. A claim's number lives ` +
    ...
    if (target.quantity !== null && !isKnown(target.quantity)) {
      refusals.push({
        ...
        message:
          `The claim '${node.claim.metric}' reads its value from '${target.label}', which is UNKNOWN. An unknown ` +
